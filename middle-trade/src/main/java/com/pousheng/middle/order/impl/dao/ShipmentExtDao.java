...
public class ShipmentExtDao extends MyBatisDao<Shipment> {
    ...
    public Paging<Shipment> findShipmentCustom(Integer status, Date updatedAtStart, Date updatedAtEnd, Long limit, Integer offset) {
        Long total = this.sqlSession.selectOne(this.sqlId("findCustomCount"), ImmutableMap.of("status", status, "startAt", updatedAtStart, "endAt", updatedAtEnd));

        if (total <= 0L) {
            return new Paging(0L, Collections.emptyList());
        } else {
            List<Shipment> shipments = this.getSqlSession().selectList(this.sqlId("findCustom"),
                    ImmutableMap.of("status", status, "startAt", updatedAtStart, "endAt", updatedAtEnd, "limit", limit, "offset", offset));
            return new Paging(total, shipments);
        }
    }
