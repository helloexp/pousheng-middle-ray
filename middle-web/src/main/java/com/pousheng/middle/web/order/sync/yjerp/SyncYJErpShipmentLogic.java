...
public class SyncYJErpShipmentLogic {
    ...
    public Response<Boolean> syncShipmentToYJErp(Shipment shipment) {
        try {
            ...
            log.info("sync shipment to yj erp result:{}", response);
    ...
    public Response<Boolean> syncShipmentCancelToYJErp(Shipment shipment) {
        try {
            ...
            } else if (Objects.equals(responseObj.get("error"), 2)) {
                log.info("发货单取消中 shipment:({}), response({})", shipment, responseObj.getString("error_info"));
    ...
    public YJErpShipmentInfo convertYJErpShipmentInfo(ShipmentDetail shipmentDetail) {
        ...
        setYJShopInfo(yjErpShipmentInfo,shopOrder);
    ...
    private void setYJShopInfo(YJErpShipmentInfo yjErpShipmentInfo,ShopOrder shopOrder) {

        yjErpShipmentInfo.setShop_channels(shopOrder.getOutFrom());
        Response<OpenShop> openshopRes = openShopReadService.findById(shopOrder.getShopId());
        if (openshopRes.isSuccess()&& Objects.nonNull(openshopRes.getResult())) {
            Map<String, String> shopExtraMap = openshopRes.getResult().getExtra();
            if (!CollectionUtils.isEmpty(shopExtraMap) ) {
                if(shopExtraMap.containsKey(TradeConstants.HK_COMPANY_CODE)){
                    yjErpShipmentInfo.setShop_companycpde(shopExtraMap.get(TradeConstants.HK_COMPANY_CODE));
                }
                if(shopExtraMap.containsKey(TradeConstants.HK_PERFORMANCE_SHOP_OUT_CODE)){
                    yjErpShipmentInfo.setShop_code(shopExtraMap.get(TradeConstants.HK_PERFORMANCE_SHOP_OUT_CODE));
                }
