...
public class PsOrderReceiver extends DefaultOrderReceiver {
    ...
    @Override
    protected Response<Boolean> updateParanaOrder(ShopOrder shopOrder, OpenClientFullOrder openClientFullOrder) {
        ...
        if (openClientFullOrder.getStatus() == OpenClientOrderStatus.CONFIRMED) {
            ...
            if (shopOrder.getExtra().containsKey(TradeConstants.IS_ASSIGN_SHOP) && Objects.equals(
                    shopOrder.getExtra().get(TradeConstants.IS_SINCE), "2")) {
                ...
                if (skuOrders.size() == 0) {
                    return Response.fail("not.found.sku");
                }
                for (SkuOrder skuOrder : skuOrders) {
                    Response<Boolean> updateRlt = orderWriteService.skuOrderStatusChanged(skuOrder.getId(),
                            MiddleOrderStatus.WAIT_SHIP.getValue(), MiddleOrderStatus.SHIPPED.getValue());
        ...
        if (openClientFullOrder.getStatus() == OpenClientOrderStatus.CANCEL && Objects.equals(shopOrder.getOutFrom(),
                MiddleChannel.VIPOXO.getValue())) {
            updateCancelOrder(shopOrder, openClientFullOrder);
            try {
                //自动触发订单取消
                log.info("try to cancel OXO order {}", shopOrder.getOrderCode());
                autoCreateRejectOrderHandler.cancelOxoOrder(shopOrder.getId());
            } catch (Exception ex) {
    ...
    public void noticeConfirm(Long shopOrderId) {
        ...
        if (!Objects.equals(shopOrder.getStatus(), MiddleOrderStatus.CONFIRMED.getValue())) {
            ...
            if (response.isSuccess()) {
                ...
                if (!result.isSuccess()) {
                    log.error("notifyHkOrderDoneLogic ctreateNotifyHkOrderDoneTask error shopOrderId ({})",
                            shopOrder.getId());
                }
                //通知mpos店发发货单确认收货
                ...
                if (!mposTaskResult.isSuccess()) {
                    log.error("notifyHkOrderDoneLogic ctreateNotifyMposDoneTask error shopOrderId ({})",
                            shopOrder.getId());
                }
            }
        }
    ...
    private void updateStepOrderInfo(ShopOrder shopOrder, OpenClientFullOrder openClientFullOrder) {
            ...
            if (Objects.nonNull(openClientFullOrder.getStepStatus())
                ...
                if (!StringUtils.isEmpty(isStepOrder) && Objects.equals(isStepOrder, "true")) {
                    if (!StringUtils.isEmpty(stepOrderStatus) && Objects.equals(stepOrderStatus,
                        ...
                        //抛出一个事件更新预售订单状态
                        log.info(
                                "start processing taobao step order,orderId {},isStepOrder {},currentStepOrderStatus {}"
        ...
        if (Objects.equals(MiddleChannel.JD.getValue(), shopOrder.getOutFrom()) && Objects.isNull(
                openClientFullOrder.getIsStepOrder())) {
            Map<String, String> extraMap = shopOrder.getExtra();
            String isStepOrder = extraMap.get(TradeConstants.IS_STEP_ORDER);
            String stepOrderStatus = extraMap.get(TradeConstants.STEP_ORDER_STATUS);
            if (!StringUtils.isEmpty(isStepOrder) && Objects.equals(isStepOrder, "true")) {
    ...
    protected RichOrder makeParanaOrder(OpenClientShop openClientShop,
        ...
        if (richSkusByShop.getExtra() != null && richSkusByShop.getExtra().containsKey(TradeConstants.IS_ASSIGN_SHOP)) {
            ...
            if (Objects.equals(tempExtra.get(TradeConstants.IS_ASSIGN_SHOP), "1")) {
                ...
                Response<Optional<Shop>> shopResponse = psShopReadService.findByOuterIdAndBusinessId(outerId,
                        Long.valueOf(companyId));
                if (!shopResponse.isSuccess()) {
                    log.error("find shop by outer id:{} and business id:{} fail,error:{}", outerId, companyId,
                            shopResponse.getError());
        ...
        Map<String, String> shopOrderExtra = richSkusByShop.getExtra() == null ? Maps.newHashMap()
                : richSkusByShop.getExtra();
        ...
        //判断是否是预售订单
        if (Objects.nonNull(openClientFullOrder.getIsStepOrder()) && openClientFullOrder.getIsStepOrder()) {
            shopOrderExtra.put(TradeConstants.IS_STEP_ORDER,
                    String.valueOf(openClientFullOrder.getIsStepOrder().booleanValue()));
            if (Objects.nonNull(openClientFullOrder.getStepStatus())) {
                shopOrderExtra.put(TradeConstants.STEP_ORDER_STATUS,
                        String.valueOf(openClientFullOrder.getStepStatus().getValue()));
            }
        ...
        //判断是否存在可用的赠品
        List<PoushengGiftActivity> activities = poushengGiftActivityReadLogic.findByStatus(
                PoushengGiftActivityStatus.WAIT_DONE.getValue());
        //获取赠品商品
        PoushengGiftActivity activity = psGiftActivityStrategy.getAvailGiftActivity(richSkusByShop,
                activities);  //最终选择的活动
        ...
        Long invoiceId = null;
        if (Arguments.notNull(openClientFullOrder.getInvoice()) && Arguments.notNull(
                openClientFullOrder.getInvoice().getType())) {
            //生成发票信息(可能生成失败)
    ...
    @Override
    protected Response<Boolean> saveParanaOrder(RichOrder richOrder) {
        ...
        if (Objects.equals(orginRichSkusByShop.getOrderStatus(), OpenClientOrderStatus.PAID.getValue())
            ...
            if (richOrder == null) {
                return Response.fail("rich.orer.null");
            }
            try {
                if (Arguments.notNull(orginRichSkusByShop.getReceiverInfo().getExtra()) && orginRichSkusByShop
                        .getReceiverInfo().getExtra().containsKey(TradeConstants.VAUGE_ADDRESS)) {
                    for (RichSkusByShop order : richOrder.getRichSkusByShops()) {
                        order.getExtra().put(TradeConstants.VAUGE_ADDRESS,
                                richOrder.getReceiverInfo().getExtra().get(TradeConstants.VAUGE_ADDRESS));
                    }
                }

            } catch (Exception e) {
                log.error("set address flag error for richorder:{} cause:{}", richOrder,
            ...
            if (!createR.isSuccess()) {
                ...
                return Response.fail("fail.to.save.order");
            }
            ...
            for (Long shopOrderId : createR.getResult()) {
                ...
                if (!r.isSuccess()) {
                    ...
                    //只有非淘宝的订单可以抛出事件
                    if (!Objects.equals(shopOrder.getOutFrom(), "taobao") && !Objects.equals(shopOrder.getOutFrom(),
                            "tfenxiao")) {
        ...
        try {
            terminusErpOpenApiClient.doPost("sync.taobao.order.recever.info.api",
                    ImmutableMap.of("shopId", richSkusByShop.getShop().getId(), "orderId", richSkusByShop.getOuterOrderId(),
                            "redirectUrl", poushengPagodaCommonRedirectUrl));
        } catch (Exception e) {
            log.error("fail to send sync order receiver request to erp for order(outOrderId={},openShopId={}),cause:{}",
                    richSkusByShop.getOuterOrderId(), richSkusByShop.getShop().getId(),
                    Throwables.getStackTraceAsString(e));
        }
    ...
    private void syncFenxiaoReceiverInfo(RichSkusByShop richSkusByShop) {
        try {
            if (log.isDebugEnabled()) {
                log.debug("sync fenxiao receiver info start,shopId {},orderId {}", richSkusByShop.getShop().getId(),
                        richSkusByShop.getOuterOrderId());
            }
           /* erpOpenApiClient.doPost("fenxiao.order.receiver.sync",
                    ImmutableMap.of("shopId", richSkusByShop.getShop().getId(), "orderId", richSkusByShop
                    .getOuterOrderId()));*/
            terminusErpOpenApiClient.doPost("sync.taobao.fenxiao.order.recever.info.api",
                    ImmutableMap.of("shopId", richSkusByShop.getShop().getId(), "orderId", richSkusByShop.getOuterOrderId(),
                            "redirectUrl", poushengPagodaFenxiaoRedirectUrl));
        } catch (Exception e) {
            log.error("fail to send sync order receiver request to erp for order(outOrderId={},openShopId={}),cause:{}",
                    richSkusByShop.getOuterOrderId(), richSkusByShop.getShop().getId(),
                    Throwables.getStackTraceAsString(e));
    ...
    protected void calculatePlatformDiscountForSkus(RichSkusByShop richSkusByShop) {
        ...
        if (!StringUtils.isEmpty(platformDiscount)) {
            ...
            for (RichSku richSku : richSkus) {
                ...
                if (richSku.getOriginFee().compareTo(richSku.getDiscount()) == 0) {
                    String key =buildUniqueKey(richSku);
                    excludeItemMap.put(key, richSku.getDiscount());
                }
            }
            Long alreadyShareDiscout = 0L;
            List<RichSku> richSkuList = richSkus.stream().filter(
                    richSku -> !excludeItemMap.containsKey(buildUniqueKey(richSku))).collect(Collectors.toList());
            for (int i = 0; i < richSkuList.size() - 1; i++) {
                ...
                skuIdAndShareDiscount.put(buildUniqueKey(richSkuList.get(i)), itemShareDiscount);
            }
            //计算剩余没有分配的平台优惠
            Long remainShareDiscount = Long.valueOf(platformDiscount) - alreadyShareDiscout;
            for (RichSku richSku : richSkuList) {
                Long shareDiscount = skuIdAndShareDiscount.get(buildUniqueKey(richSku));
                Map<String, String> skuExtra = richSku.getExtra();
                //子订单插入平台优惠金额
    ...
    public static String buildUniqueKey(RichSku richSku ) {

        if(!Objects.equals(richSku.getOuterSkuId(),"0")){
            ItemUtils.buildUniqueKey(richSku.getOuterSkuId(),"");
            return richSku.getOuterSkuId();
        }

        if(richSku.getExtra()!=null){
            if(!StringUtils.isEmpty(richSku.getExtra().get("outItemId"))){
                return richSku.getExtra().get("outItemId");
            }
        }

        if(!StringUtils.isEmpty(richSku.getOuterOrderId())){
            return richSku.getOuterOrderId();
        }
        //找不到可以唯一的东东了
        return ItemUtils.buildUniqueKey(richSku.getOuterSkuId(),"");
    ...
    public void syncReceiverInfo(ReceiverInfoDecryptDTO dto) {
        try {
            terminusErpOpenApiClient.doPost("sync.taobao.order.recever.info.api",
                    ImmutableMap.of("shopId", dto.getShopId(), "orderId", dto.getOutId(), "redirectUrl",
                            poushengPagodaCommonRedirectUrl));
        } catch (Exception e) {
            log.error("fail to send sync order receiver request to erp for order(outOrderId={},openShopId={}),cause:{}",
    ...
    public void syncFenxiaoReceiverInfo(ReceiverInfoDecryptDTO dto) {
        try {
            ...
            terminusErpOpenApiClient.doPost("sync.taobao.fenxiao.order.recever.info.api",
                    ImmutableMap.of("shopId", dto.getShopId(), "orderId", dto.getOutId(), "redirectUrl",
                            poushengPagodaFenxiaoRedirectUrl));
        } catch (Exception e) {
            log.error("fail to send sync order receiver request to erp for order(outOrderId={},openShopId={}),cause:{}",
                    dto.getOutId(), dto.getShopId(), Throwables.getStackTraceAsString(e));
