...
public class SyncYJErpReturnLogic {
    ...
    @Autowired
    private SycYunjuRefundOrderApi sycYunjuRefundOrderApi;
    ...
    public Response<Boolean> syncRefundToYJErp(Refund refund) {
        ...
        try {
            ...
            } else if (Objects.equals(responseObj.get("error"), 2)) {//取消中
                log.info("订单派发中心返回信息取消中  refund:({}), error:({})", refund, responseObj.getString("error_info"));
     ...
     * @return
     */
    public Response<Boolean> syncRejectRefundCanalToYjErp(Refund refund) {
        try {
            log.info("云聚拒收单同步:{}", refund.getRefundCode());
            ...
            Integer targetStatus = flow.target(refund.getStatus(), orderOperation);
            refund.setStatus(targetStatus);
            YJRespone yjRespone = sycYunjuRefundOrderApi.doSyncRejectRefundCancel(YJSyncRejectRefundCancelRequest.builder()
                    .mg_exchange_sn(refund.getRefundCode())
                    .build());
            //0取消成功
            ...
            //2取消中
            else if (yjRespone.getError() == 2) {
                //将状态保持为取消中，等待云聚接口异步回调回来
                //updateRefundCancelSyncFail(refund);
                log.info("云聚拒收单取消返回信息:{}", JSON.toJSONString(yjRespone));
            }
            //1 取消失败
            else {
                updateRefundCancelSyncFail(refund);
                log.error("云聚拒收单取消返回信息:{}", JSON.toJSONString(yjRespone));
                return Response.fail("云聚拒收单取消返回信息:" + yjRespone.getError_info());
            }
        } catch (Exception e) {
            log.error("sync yj erp reject refund cancel failed,refundId is({}) cause by({})", refund.getId(), Throwables.getStackTraceAsString(e));
            //更新同步状态
            updateRefundCancelSyncFail(refund);
            return Response.fail("sync.yjerp.refund.fail");
        }
    ...
    public List<YJErpRefundProductInfo> getYJErpRefundProductInfo(Refund refund) {
        ...
        refundItemList.forEach(item -> {
            ...
            // TODO 默认客户拒收 拒收单类型
            if (Objects.equals(MiddleRefundType.REJECT_GOODS.value(), refund.getRefundType())) {
    ...
    private void updateRefundCancelSyncFail(Refund refund) {
        OrderOperation orderOperation = MiddleOrderEvent.SYNC_CANCEL_FAIL.toOrderOperation();
        Response<Boolean> updateSyncStatusRes = refundWriteLogic.updateStatusLocking(refund, orderOperation);
        if (!updateSyncStatusRes.isSuccess()) {
            log.error("refund(id:{}) operation :{} fail,error:{}", refund.getId(), orderOperation.getText(), updateSyncStatusRes.getError());
        }
