...
import io.terminus.search.api.query.Criteria;
...
public class Shipments {
    ...
    public List<Long> createAfterShipment(@PathVariable("id") Long refundId,
        ...
        for (ShipmentRequest shipmentRequest : requestDataList) {
            try {
                ...
                // XXX RAY POUS925 2019.04.18: 設定Shipment快遞商ID和名稱
                RefundExtra refundExt = refundReadLogic.findRefundExtra(refund);
				if (StringUtils.hasText(refundExt.getNewShipmentCorpCode())) {					
    ...
    public void syncHkShipment(@PathVariable(value = "id") Long shipmentId) {
        ...
        if(Objects.equals(MiddleChannel.JD.getValue(), shopOrder.getOutFrom()) || Objects.equals(MiddleChannel.TAOBAO.getValue(), shopOrder.getOutFrom())) {
    ...
    private List<ShipmentItem> makeShipmentItems(List<SkuOrder> skuOrders, Map<Long, Integer> skuOrderIdAndQuantity,
        ...
        for (Long skuOrderId : skuOrderIdAndQuantity.keySet()) {
            ...
            WarehouseDTO warehouse = findWarehouseById(warehouseId);
            if (Objects.equals(warehouse.getWarehouseSubType(), WarehouseType.TOTAL_WAREHOUSE.value())) {
                shipmentItem.setWarehouseId(warehouse.getId());
            } else {
                Shop shop = middleShopCacher.findByOuterIdAndBusinessId(warehouse.getOutCode(), Long.valueOf(warehouse.getCompanyId()));
...
//            String shipmentWay = shipment.getExtra().get("shipmentWay");
    ...
    public void updateShipmentsAmountByShopOrderId(@PathVariable(value = "shopId") Long shopId, @RequestParam("shopOrderId") Long shopOrderId) {
        ...
        shipmentWiteLogic.updateShipmentFee(shopOrderId);
    }
    ...
    @ApiOperation("被拒历史,3天内")
    @GetMapping(value = "/api/shipment/paging/reject/reason", produces = MediaType.APPLICATION_JSON_VALUE)
    public Response<Paging<ShipmentRejectReason>> pagingRejectReason(Integer pageNo, Integer pageSize,
                                                                     @RequestParam Long warehouseId,
                                                                     @RequestParam String skuCode) {
        Map<String, Object> criteria = Maps.newHashMap();
        criteria.put("warehouseId", warehouseId);
        criteria.put("skuCode", skuCode);
        criteria.put("day", DateTime.now().minusDays(3).toDate());
        return middleShipmentReadService.pagingRejectReason(pageNo, pageSize, criteria);
    }
}