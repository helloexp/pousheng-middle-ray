...
public class AdminOpenShops {
    ...
    private void editExtraInfo (OpenShop openShop, Map<String, String> map) {
        ...
        if (jsonMap.containsKey(TradeConstants.SHOW_OVER)) {
            // 是否启用发货时效
            map.put(TradeConstants.SHOW_OVER, jsonMap.get(TradeConstants.SHOW_OVER));
        }
        if (jsonMap.containsKey(TradeConstants.OVER_TYPE_1)) {
            // 发货时效1
            map.put(TradeConstants.OVER_TYPE_1, jsonMap.get(TradeConstants.OVER_TYPE_1));
        }
        if (jsonMap.containsKey(TradeConstants.OVER_TYPE_2)) {
            // 发货时效2
            map.put(TradeConstants.OVER_TYPE_2, jsonMap.get(TradeConstants.OVER_TYPE_2));
        }
        if(Objects.equals(MiddleChannel.TAOBAO.getValue(), openShop.getChannel())) {
            if (jsonMap.containsKey(TradeConstants.IS_TAOBAO_SHOP)) {
                // 拉取映射关系
