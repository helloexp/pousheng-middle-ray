...
public class PsAfterSaleReceiver extends DefaultAfterSaleReceiver {
     ...
     * @param refundExtra
     */
    private List<Refund> fillRefundItem(ShopOrder shopOrder, Refund refund, List<SkuOfRefund> skuOfRefundList, RefundExtra refundExtra) {
        try {
            ...
            skuOfRefundList.forEach(skuOfRefund ->
            {
                ...
                if (!Objects.isNull(shipment)) {
                    ...
                    ShipmentItem shipmentItem = shipmentItems.stream()
                            .filter(si -> Objects.equals(SkuCodeUtil.getCombineCode(si), SkuCodeUtil.getCombineCode(refundItem)))
                            .findFirst().get();
                    ...
                    if (!Objects.equals(refund.getRefundType(), MiddleRefundType.ON_SALES_REFUND.value()) &&
                            !Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_REFUND.value())) {
                        updateShipmentItemRefundQuantity(SkuCodeUtil.getCombineCode(refundItem), shipmentItem.getQuantity(), shipmentItems);
                    }
                    //更新发货单商品中的已退货数量
     ...
     * @param refundExtra
     */
    private List<Refund> fillRefundItem(ShopOrder shopOrder, Refund originRefund, SkuOfRefund skuOfRefund, RefundExtra refundExtra) {
        List<Refund> refundList = Lists.newArrayList();
        try {
            SkuOrder skuOrder;
            if (StringUtils.hasText(skuOfRefund.getChannelSkuId())) {
    ...
    private Shipment findShipmentByOrderInfo(long shopOrderId, String skuCode, Integer quantity) {
        ...
        for (Shipment shipment : shipments) {
            ...
            List<ShipmentItem> shipmentItemFilters = shipmentItems.stream().
                    filter(Objects::nonNull).filter(shipmentItem -> Objects.equals(SkuCodeUtil.getCombineCode(shipmentItem), skuCode))
