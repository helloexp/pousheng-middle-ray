...
public class SyncErpReturnLogic {
    ...
    public Response<Boolean> syncReturn(Refund refund){
        ...
        // 共享仓云聚售后拒收类型单，调整为退货退款、拒收单都同步云聚
        List<Integer> jitRefundSyncTypes = Lists.newArrayList(
                MiddleRefundType.REJECT_GOODS.value(),
                MiddleRefundType.AFTER_SALES_RETURN.value()
        );
        if (Objects.equals(shipment.getShipWay(),2) && jitRefundSyncTypes.contains(refund.getRefundType())){
    ...
    public Response<Boolean> syncReturnCancel(Refund refund){
        ...
        if(jitRefundSyncTypes.contains(refund.getRefundType())){
            RefundExtra refundExtra = refundReadLogic.findRefundExtra(refund);
            Shipment shipment =  shipmentReadLogic.findShipmentByShipmentCode(refundExtra.getShipmentId());
            if(shipment.getShipWay() == 2 && syncYJErp(shipment.getShipId())){
                return syncYJErpReturnLogic.syncRejectRefundCanalToYjErp(refund);
            }
        }
        OpenShop openShop = openShopResponse.getResult();
        Map<String, String> openShopExtra =  openShop.getExtra();
