...
DROP TABLE IF EXISTS `parana_refunds_templates`;
CREATE TABLE `parana_refunds_templates` (
  `id` bigint(11) unsigned NOT NULL AUTO_INCREMENT,
  `batch_code` varchar(55) NOT NULL COMMENT '导入批次码',  
  `sale_code` varchar(25) DEFAULT NULL COMMENT '交易单号',
  `ship_code` varchar(25) DEFAULT NULL COMMENT '发货单号',
  `sku_code` varchar(55) DEFAULT NULL COMMENT '商品条码',
  `num` int(10) DEFAULT NULL COMMENT '商品数量',
  `apply_status` int(4) DEFAULT NULL COMMENT '创建售后单：1，成功；0，失败',
  `created_at` datetime DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='创建售后单导入模板';
