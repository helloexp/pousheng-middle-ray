    ...
    protected void oneBiz(YyEdiShipInfo yyEdiShipInfo) {
        ...
        if (yyEdiShipInfo.getItemInfos() != null) {
            ...
            if (fromJit) {
                ...
                //校验jit订单是否整单缺货
                jitOutOfStockSyncYjHandler.checkWmsCallbackResult(yyEdiShipInfo);
                ...
                for (ShipmentItem s : items) {
                    ...
                    if (partShip) {
                        assignJitShipmentQuantity(s, quantityTable, assignBoxTable);
                    } else {
                        partShip = assignJitShipmentQuantity(s, quantityTable, assignBoxTable);
                    }
                }
            } else {
                ...
                for (ShipmentItem s : items) {
                    //若已经标记了部分发货 则后续不覆盖此标志位
                    if (partShip) {
                        assignShipmentQuantity(s, itemMap);
                    } else {
                        partShip = assignShipmentQuantity(s, itemMap);
                    }
                }
            }
        } else {
...
//        //guofeng 添加oxo订单同步发货信息至ESP的逻辑
//        if (MiddleShipmentsStatus.SHIPPED.getValue() == targetStatus) {
//            OpenShop openShop = openShopCacher.findById(shipment.getShopId());
//            if (MiddleChannel.VIPOXO.getValue().equals(openShop.getChannel())) {
//                WarehouseShipmentShipedEvent event = new WarehouseShipmentShipedEvent();
//                event.setShipmentId(shipmentId);
//                event.setExpressbillno(yyEdiShipInfo.getShipmentSerialNo());
//                event.setExpresscode(yyEdiShipInfo.getShipmentCorpCode());
//                eventBus.post(event);
//            }
//        }
        ...
        //oxo自动创建拒收单
        shipment = shipmentReadLogic.findShipmentByShipmentCode(shipmentCode);
