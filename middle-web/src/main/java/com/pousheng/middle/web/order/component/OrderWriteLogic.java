...
public class OrderWriteLogic {
    ...
    public Response<Boolean> updateEcpOrderStatus(ShopOrder shopOrder, OrderOperation orderOperation) {
        // 中台手工创建订单标记，在发货完成后不走SYNC_ECP回执电商平台
        if (shopOrder.getExtra().containsKey(ExtraKeyConstant.IS_ARTIFICIAL)
                && Objects.equals("Y", shopOrder.getExtra().get(ExtraKeyConstant.IS_ARTIFICIAL))
                && Objects.equals(orderOperation, MiddleOrderEvent.SYNC_FAIL.toOrderOperation())) {
            orderOperation = MiddleOrderEvent.SYNC_SUCCESS.toOrderOperation();
        }
