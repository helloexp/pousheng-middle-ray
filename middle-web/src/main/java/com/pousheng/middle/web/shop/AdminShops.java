...
public class AdminShops {
	...
	/**
	 * 2019.03.29 RAY: 增加快遞公司篩選 分页查询门店信息
	 * 
	 * @param id              id
	 * @param name            名称
	 * @param userId          用户id
	...
	public Paging<ShopPaging> paging(@RequestParam(required = false) Long id,
			...
			@RequestParam(required = false) String expresssCompany, @RequestParam(required = false) Integer pageNo,
		...
		Response<Paging<Shop>> resp = null;
		if (StringUtils.isEmpty(expresssCompany)) { // 沒有傳入快遞公司，呼叫原邏輯
			resp = psShopReadService.pagination(name, userId, type, status, outerId, companyId, zoneIds, pageNo,
					pageSize);
		} else { // 傳入快遞公司，呼叫新邏輯
			resp = psShopReadService.pagingWithExpresssCompany(name, userId, type, status, outerId, companyId, zoneIds,
					expresssCompany, pageNo, pageSize);
		}
		if (!resp.isSuccess()) {
