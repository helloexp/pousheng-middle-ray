...
public class RefundWriteLogic {
    ...
    private Long forwardRefundWareHouseId;

    @Autowired
    private CompensateBizLogic compensateBizLogic;
    // 2019.09.09 RAＹ：　修改方法對應的快遞公司
    // 2019.4.11 RAY: 讀取快遞公司訊息
    @Autowired
    private ExpressCodeReadService expressCompanyReadSer;
    ...
    public Long createRefund(SubmitRefundInfo submitRefundInfo) {
		...
		// XXX RAY POUS925 2019.04.11: 售后换货，設定物流公司代码，名稱
		if (Objects.equals(submitRefundInfo.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value())) {
			setRefundExtraBean(refundExtra, submitRefundInfo.getNewShipmentCorpName());
		}
    ...
    public void completeHandle(Refund refund, EditSubmitRefundInfo submitRefundInfo) {
		...
		// XXX RAY POUS925 2019.05.09: 售后换货，設定物流公司代码，名稱
        ...
        //实例化SubmitRefundInfo对象，并赋予submitRefundInfo的值
        SubmitRefundInfo refundInfo = new SubmitRefundInfo();
        BeanUtils.copyProperties(submitRefundInfo, refundInfo);
        refundInfo.setRefundType(refund.getRefundType());
        //设置已经存在的售后商品信息
        refundInfo.setExistRefundItems(existRefundItems);

        //申请数量是否有效,传的是EditSubmitRefundInfo的子类对象
        currentRefundItems = checkRefundQuantity(refundInfo, shipmentItems);
    ...
    public Long createYunJURefund(SubmitRefundInfo submitRefundInfo) {
        ...
        refundExtra.setShipmentId(shipment.getShipmentCode());
        //完善退货仓库及物流信息
        completeYunJUWareHoseAndExpressInfo(shopOrder, submitRefundInfo.getRefundType(), refundExtra, submitRefundInfo, shipment);
        refund.setShipmentSerialNo(refundExtra.getShipmentSerialNo());
        refund.setShipmentCorpCode(refundExtra.getShipmentCorpCode());
    ...
    //完善云聚退货仓信息
    private void completeYunJUWareHoseAndExpressInfo(ShopOrder shopOrder, Integer refundType, RefundExtra refundExtra, EditSubmitRefundInfo submitRefundInfo, Shipment shipment) {
        ...
        if (!Objects.equals(refundType, MiddleRefundType.AFTER_SALES_REFUND.value())) {
            ...
            if (Objects.equals(refundType, MiddleRefundType.REJECT_GOODS.value())) {
                ShipmentExtra shipmentExtra = shipmentReadLogic.getShipmentExtra(shipment);
                //填入售后单仓库id
                refundWarehouseId = shipmentExtra.getWarehouseId().toString();
                //填入售后单仓库名称
                refundWarehouseName = shipmentExtra.getWarehouseName();
                //拒收单设置发货单的
     ...
     * @return
     */
    private List<RefundItem> checkRefundQuantity(SubmitRefundInfo submitRefundInfo, List<ShipmentItem> shipmentItems) {
        final Integer refundType;
        refundType = submitRefundInfo.getRefundType();
        //当前售后单已申请商品信息
        List<RefundItem> existRefundItems = submitRefundInfo.getExistRefundItems();
        Map<String, Integer> existSkuCodeAndQuantity = new HashMap<>();
        if (Objects.nonNull(existRefundItems) && !existRefundItems.isEmpty()) {
            for (RefundItem refundItem : existRefundItems) {
                String key = SkuCodeUtil.getCombineCode(refundItem);
                Integer quantity = MoreObjects.firstNonNull(existSkuCodeAndQuantity.get(key), 0)
                        + MoreObjects.firstNonNull(refundItem.getApplyQuantity(), 0);
        ...
        //申请的退货skuId-quantity集合
        Map<Long, Integer> editSkuCodesAndQuantity = Maps.newHashMap();
        editSubmitRefundItems.forEach(editSubmitRefundItem -> {
            editSkuCodesAndQuantity.put(editSubmitRefundItem.getSkuOrderId(), editSubmitRefundItem.getRefundQuantity() == null ? 0 : editSubmitRefundItem.getRefundQuantity());
        });
        //拒收单只能是整单拒收
        if (Objects.equals(refundType, MiddleRefundType.REJECT_GOODS.value())) {
            shipmentItems.forEach(shipmentItem -> {
                if (editSkuCodesAndQuantity.get(shipmentItem.getSkuOrderId()) == null) {
                    log.error("refund applySkuId:{} invalid", shipmentItem.getSkuOrderId());
        ...
        for (EditSubmitRefundItem editSubmitRefundItem : editSubmitRefundItems) {
            ...
            if (skuCodes.contains(key)) {
                ...
                int existRefundApplyQuantity = existSkuCodeAndQuantity.get(key) == null ? 0 : existSkuCodeAndQuantity.get(key);
                ...
                if (Objects.equals(refundType, MiddleRefundType.REJECT_GOODS.value())) {
                    if (currentRefundApplyQuantity != remainQuantuty) {
                        log.error("refund applyQuantity:{} gt available applyQuantity:{}", editSubmitRefundItem.getRefundQuantity(), remainQuantuty);
                        throw new JsonResponseException("refund.apply.quantity.invalid");
                    }
                } else if ((currentRefundApplyQuantity + alreadyRefundApplyQuantity - existRefundApplyQuantity) > remainQuantuty) {
                    log.error("refund applyQuantity:{} gt available applyQuantity:{}", editSubmitRefundItem.getRefundQuantity(), remainQuantuty);
                    throw new JsonResponseException("refund.apply.quantity.invalid");
                }
    ...
    private List<RefundItem> checkYunJuRefundQuantity(EditSubmitRefundInfo submitRefundInfo, List<ShipmentItem> shipmentItems) {
        ...
        Map<String, ShipmentItem> skuCodesAndShipmentItems = Maps.newHashMap();
        shipmentItems.forEach(shipmentItem -> {
            //云聚下发没有skuOrderId，暂且使用skuCode处理，若出现重复最好协调云聚参数增加skuOrderId
            //String outSkuCode = SkuCodeUtil.getCombineCode(shipmentItem);
            String outSkuCode = shipmentItem.getSkuCode();
    ...
    public Long createRefundForLost(SubmitRefundInfo submitRefundInfo) {
		...
		// XXX RAY POUS925 2019.04.11: 丢件补发，設定物流公司代码，名稱
		setRefundExtraBean(refundExtra, submitRefundInfo.getNewShipmentCorpName());
    ...
    public void completeHandleForLostType(Refund refund, EditSubmitRefundInfo submitRefundInfo) {
        ...
        //添加处理完成时间
        refundExtra.setHandleDoneAt(new Date());
		// XXX RAY POUS925 2019.05.09: 丢件补发，設定物流公司代码，名稱
		this.setRefundExtraBean(refundExtra, submitRefundInfo.getNewShipmentCorpName());
    ...
    private List<ShipmentRequest> groupByWarehouseId(List<EditSubmitChangeItem> editSubmitChangeItems) {
        ...
        for (Long warehouseId : warehouseMulitMaps.keySet()) {
            ...
            for (EditSubmitChangeItem editSubmitChangeItem : changeItems) {
                ...
                data.put(SkuCodeUtil.toCombineCode(editSubmitChangeItem), editSubmitChangeItem.getChangeQuantity());
            }
    ...
    private boolean createOccupyShipment(ShipmentRequest shipmentRequest, Long refundId) {
        try {
            ...
            if (!refundReadLogic.checkRefundWaitHandleNumber2(refundChangeItems, skuCodeAndQuantity)) {
            ...
            checkStockIsEnough2(warehouseId, skuCodeAndQuantity, openShop.getId());
			...
			// XXX RAY POUS925 2019.09.09: 設定Shipment快遞商ID和名稱
			RefundExtra refundExt = refundReadLogic.findRefundExtra(refund);
			if (Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value())) {
				shipment.setShipmentCorpCode(refundExt.getNewShipmentCorpCode());
				shipment.setShipmentCorpName(refundExt.getNewShipmentCorpName());
				// 寫入vendCustID
				ShipmentExtra shipExt = shipmentReadLogic.getShipmentExtra(shipment);
				shipExt.setVendCustID(refundExt.getNewShipmentCorpCode());
     ...
     * @param shopId                店铺id
     */
    private void checkStockIsEnough2(Long warehouseId, Map<String, Integer> skuCodeAndQuantityMap, Long shopId) {
        Set<String> skuCodes = skuCodeAndQuantityMap.keySet().stream().map(key -> key.split(":")[1]).collect(Collectors.toSet());
        Map<String, Integer> warehouseStockInfo = findStocksForSkus(warehouseId, Lists.newArrayList(skuCodes), shopId);
        //存在skuCode重复的情况
        if (skuCodes.size() < skuCodeAndQuantityMap.size()) {
            //计算重复skuCode的总数量
            Map<String, Integer> skuCodeQuantitys = Maps.newHashMap();
            for (Map.Entry<String, Integer> entry : skuCodeAndQuantityMap.entrySet()) {
                String skuCode = entry.getKey().split(":")[1];
                if (!skuCodeQuantitys.containsKey(skuCode)) {
                    skuCodeQuantitys.put(skuCode, entry.getValue());
                } else {
                    skuCodeQuantitys.put(skuCode, skuCodeQuantitys.get(skuCode) + entry.getValue());
                }
            }
            for (String skuCode : warehouseStockInfo.keySet()) {
                if (warehouseStockInfo.get(skuCode) < skuCodeQuantitys.get(skuCode)) {
                    log.error("sku code:{} warehouse stock:{} ship applyQuantity:{} stock not enough",
                            skuCode, warehouseStockInfo.get(skuCode), skuCodeQuantitys.get(skuCode));
                    throw new JsonResponseException(skuCode + ".stock.not.enough");
                }
            }
        } else {
            for (Map.Entry<String, Integer> entry : skuCodeAndQuantityMap.entrySet()) {
                String skuCode = entry.getKey().split(":")[1];
                if (warehouseStockInfo.get(skuCode) < entry.getValue()) {
                    log.error("sku code:{} warehouse stock:{} ship applyQuantity:{} stock not enough",
                            skuCode, warehouseStockInfo.get(skuCode), entry.getValue());
                    throw new JsonResponseException(skuCode + ".stock.not.enough");
                }
            }
        }
    ...
    private void checkRefundCompanyCode(Shipment shipment, ShopOrder shopOrder, SubmitRefundInfo submitRefundInfo) {
        ...
        String getShipmentcompanyCode = "";
        if (Objects.equals(shipment.getShipWay().toString(), TradeConstants.MPOS_SHOP_DELIVER)) {
            // 校验店发账套
            getShipmentcompanyCode = getCompanyCodeShopDeliver(oriDeliverId);
            ...
            if (shipmentWarehousedto != null) {
                getShipmentcompanyCode = shipmentWarehousedto.getCompanyId();
            }
        }    
        if (!StringUtils.hasText(getShipmentcompanyCode)) {
                //查询异常,返回
                log.error("get shop deliver warehouse company code failed, oriDeliverid:{}", oriDeliverId);
                throw new JsonResponseException("获取发货店、仓账套失败");
        }
        // 发货仓账套为325时，后台更新退货仓为WH001092。2019-4月份        
        Response<RefundWarehouseRules> refundWarehouseRule = refundWarehouseRulesReadService.findByShopIdAndShipmentCompanyId(shopOrder.getShopId(), getShipmentcompanyCode);
        if (refundWarehouseRule.isSuccess()) {
            if (refundWarehouseRule.getResult() != null) {
                // 第二次延伸开发：发货仓账套在配置列表中时，后台更新退货仓为refundWarehouseRule.getResult().getRefundWarehouseCode()。2019-8月份扩展需求
                String warehouseCode = refundWarehouseRule.getResult().getRefundWarehouseCode();
                WarehouseDTO warehousedto = refundReadLogic.findWarehouseByCodeandCompanyId(warehouseCode);
                submitRefundInfo.setWarehouseId(warehousedto.getId());
            } else {
                WarehouseDTO refundWarehousedto = findWarehouseById(submitRefundInfo.getWarehouseId());
                String performanceShopOutCode1 = getCompanyCodeOrderShop(shopOrder.getShopId());
                // 场景2：如果来源单的发货店账套不是325，则校验来源单的店铺账套与退货仓的账套是否一致；2019-4月份
                // 第二次延伸开发：不仅仅针对325账套的，所有在配置列表中的都适用此逻辑。2019-8月份扩展需求
                if (!Objects.equals(refundWarehousedto.getCompanyId(), performanceShopOutCode1)) {
                        log.error("orderShop warehouse(warehouse company:{}) of refund(companycode:{}) companyCode is different ", refundWarehousedto.getCompanyId(), performanceShopOutCode1);
                        throw new JsonResponseException("退货仓账套与订单店铺账套不一致");
                }
            }
        } else {
            //查询异常,返回
            log.error("get RefundWarehouseRules failed,OrderShopId:{}, shipmentcompanyCode:{}", shopOrder.getShopId(), getShipmentcompanyCode);
            throw new JsonResponseException("获取退货到指定退货仓规则失败");
        }
    }
     ...
     *  
     */
    public String getCompanyCodeShopDeliver(Long shopId) {
        String companyCode = "";
     ...
     * 
     */    
    public String getCompanyCodeOrderShop(Long shopId) {
        String companyCode = "";
    ...
    /**
     * 根据店铺id查询店铺code：账套-外码
     * 
     */    
    public String findOpenShopCodeById(Long shopid){
        String performanceOutCode = "";
        Response<OpenShop> openshopRes = openShopReadService.findById(shopid);
        if (openshopRes.isSuccess()) {
            if (openshopRes.getResult() != null) {
                Map<String, String> shopExtraMap = openshopRes.getResult().getExtra();
                if (!CollectionUtils.isEmpty(shopExtraMap) && shopExtraMap.containsKey(TradeConstants.HK_COMPANY_CODE) 
                        && shopExtraMap.containsKey(TradeConstants.HK_PERFORMANCE_SHOP_OUT_CODE)) {
                   String companyCode = shopExtraMap.get(TradeConstants.HK_COMPANY_CODE);
                   String shopOutCode = shopExtraMap.get(TradeConstants.HK_PERFORMANCE_SHOP_OUT_CODE);
                   performanceOutCode = companyCode + "-" + shopOutCode;
                }
            }
        }
        return performanceOutCode;
    }
     ...
     * @param id
     */
    public void deleteRefundWarehouseRules(Long id) {
       Response<Boolean> response = refundWarehouseRulesWriteService.deleteRulesById(id);
       if(!response.isSuccess()){
           log.error("delete refundWarehouseRules failed, id:{}",id);
           throw new JsonResponseException(response.getError());
       }
    }
     ...
     * @return
     */
    public void updateRefundWarehouseRule(RefundWarehouseRules refundWarehouseRules) {
        if(!StringUtils.hasText(refundWarehouseRules.getOrderShopCode()) || !StringUtils.hasText(refundWarehouseRules.getShipmentCompanyId())
                 || !StringUtils.hasText(refundWarehouseRules.getRefundWarehouseCode()) || refundWarehouseRules.getId() == null){
            throw new JsonResponseException("编辑数据不能为空");
        }
        Response<RefundWarehouseRules> response = refundWarehouseRulesReadService.findByShopIdAndShipmentCompanyId(refundWarehouseRules.getShopId(),refundWarehouseRules.getShipmentCompanyId());
        if(!response.isSuccess()){
            throw new JsonResponseException(response.getError());
        }
        if(response.getResult() != null){
            // OrderShopCode和ShipmentCompanyId为联合唯一字段，存在重复则不允许更新。
            if(!Objects.equals(response.getResult().getId(),refundWarehouseRules.getId())){
                log.error("编辑售后指定退货仓联合唯一字段重复，refundWarehouseRulesId:{}",refundWarehouseRules.getId());
                throw new JsonResponseException("提交数据重复，该销售店铺和发货账套规则已存在");
            }
        }
        Response<Boolean> updateResponse = refundWarehouseRulesWriteService.updateRefundWarehouseRules(refundWarehouseRules);
    ...
    private List<ShipmentItem> makeChangeShipmentItems(List<RefundItem> refundChangeItems, Map<String, Integer> skuCodeAndQuantity, Long warehouseId, ShopOrder shopOrder) {
        ...
        Map<String, RefundItem> refundItemMap = refundChangeItems.stream().filter(Objects::nonNull).collect(Collectors.toMap(SkuCodeUtil::toCombineCode, it -> it));
	...
	/**
	 * XXX RAY POUS925 2019.04.15: 設定快遞公司代碼，名稱
	 * 
	 * @param bean                RefundExtra
	 * @param newShipmentCorpName 要修改的快遞公司
	 */
	private void setRefundExtraBean(RefundExtra bean, String newShipmentCorpName) {
		// 2019.4.11 RAY: 設定物流公司代码，名稱
		if (StringUtils.isEmpty(newShipmentCorpName)) {
			return;
		}
		Response<ExpressCode> expressResp = expressCompanyReadSer.findByName(newShipmentCorpName);
		String expressCode = null;
		if (expressResp.isSuccess() && expressResp.getResult() != null) {
			expressCode = expressResp.getResult().getOfficalCode();
		} else { // 查無對應的快遞公司代碼
			log.error("there is no any express info by name:{}", newShipmentCorpName);
			throw new InvalidException(500, "express.info.is.not.exist(expressName={0})", newShipmentCorpName);
		}
		bean.setNewShipmentCorpCode(expressCode); // 設定物流公司代码
		bean.setNewShipmentCorpName(newShipmentCorpName); // 設定物流公司名稱
	}
}
