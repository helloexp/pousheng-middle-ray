        ...
        public void run() {
            if (!CollectionUtils.isEmpty(autoCompensations)) {
                autoCompensations.forEach(autoCompensation -> {
                    ...
                    //增加jit单子缺货同步云聚重试任务类型
                    if(Objects.equals(autoCompensation.getType(),TradeConstants.FAIL_SYNC_OUTOFSTOCK_JIT_ORDER_TO_YJ)){
                        Map<String, String> extra = autoCompensation.getExtra();
                        if (Objects.nonNull(extra.get("param"))) {
                            Map<String, Object> param = mapper.fromJson(extra.get("param"), mapper.createCollectionType(HashMap.class, String.class, Object.class));
                            Long shopId = Long.valueOf(param.get("shopId").toString());
                            YJSyncShipmentRequest yjSyncShipmentRequest = JSON.parseObject(param.get("syncShipmentRequest").toString(),YJSyncShipmentRequest.class);
                            Response<Boolean> response = jitOutOfStockSyncYjHandler.syncOutOfStockOrder2Yj(yjSyncShipmentRequest,shopId);
                            if (response.isSuccess()) {
                                autoCompensateLogic.updateAutoCompensationTask(autoCompensation.getId());
                            } else {
                                autoCompensation.getExtra().put("error", response.getError());
