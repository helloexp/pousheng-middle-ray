...
public class ShipmentWiteLogic {
    ...
    /**
     * 是否开启所有寻源方式的重试，默认只开启toDispatch的
     */
    @Value("${dispatch.retry.allchannel:false}")
    private boolean dispatchRetryAllChannel;
    ...
    public void doAutoCreateShipment(ShopOrder shopOrder) {
        ...
        if (orderReadLogic.isAllChannelOpenShop(shopOrder.getShopId()) || isNeedMposDispatch(shopOrder)) {
            ...
            if (dispatchRetryAllChannel) {
                this.autoCreateShipmentLogicRetry(shopOrder, skuOrders);
            } else {
    ...
    public Response<String> autoHandleAllChannelOrderByNewLogic(ShopOrder shopOrder) {
        ...
        if (dispatchRetryAllChannel) {
            if (!dispatchOrderHandler.autoCreateAllChannelShipment(shopOrder, skuOrders, true)) {
    ...
    private boolean autoCreateAllChannelShipmentNewLogic(ShopOrder shopOrder, List<SkuOrder> skuOrders) {
        ...
        //存在创建失败的发货单
        boolean createShipmentFail = false;
        ...
        if (orderNoteCount > 0) {
            ...
            if (createShipmentFail) {
                this.updateShipmentNote(shopOrder, OrderWaitHandleType.STOCK_LOCK_FAIL.value());
            } else if (partSend) {
                this.updateShipmentNote(shopOrder, OrderWaitHandleType.STOCK_NOT_ENOUGH.value());
            } else {
    ...
    private Response<String> autoHandleNotAllChannelOrder(ShopOrder shopOrder) {
        ...
        if (dispatchRetryAllChannel) {
            if (!this.autoCreateShipmentLogicRetry(shopOrder, skuOrders)) {
                return Response.fail(OrderWaitHandleType.STOCK_NOT_ENOUGH.getDesc());
            }
        } else {
    ...
    private boolean autoCreateShipmentLogicRetry(ShopOrder shopOrder, List<SkuOrder> skuOrders) {
        return dispatchOrderHandler.autoCreateAllChannelShipment(shopOrder, skuOrders, false);
    }
     ...
     */
    public Long createShipment(ShopOrder shopOrder, List<SkuOrder> skuOrders, WarehouseShipment warehouseShipment) {
        if (log.isDebugEnabled()) {
     ...
     */
    public Long createShopShipment(ShopOrder shopOrder, List<SkuOrder> skuOrders, ShopShipment shopShipment) {
        if (log.isDebugEnabled()) {
     ...
     */
    public SkuOrder getSkuOrder(List<SkuOrder> skuOrders, Long skuOrderId, String skuCode) {
    ...
    public List<ShipmentItem> makeShipmentItems(List<SkuOrder> skuOrders, Map<Long, Integer> skuOrderIdAndQuantity,
        ...
        for (Long skuOrderId : skuOrderIdAndQuantity.keySet()) {
            ...
            shipmentItem.setSharePlatformDiscount(StringUtils.isNotEmpty(skuPlatformDiscount) ? Integer.valueOf(skuPlatformDiscount) : 0);
            //积分
...
//                }
//            } catch (Exception e) {
//                log.error("shop dispatch order fail, sku info {}, order id {}", shopShipment.getSkuCodeAndQuantities(), shopOrder.getId(), e);
//                createShipmentFail = true;
//            }
//        }
//
...
//            }
//        }
        dispatchOrderHandler.toDispatchOrderNew(shopOrder, skuCodeAndQuantities);
    }
    ...
    public void handleSyncShipment(Shipment shipment, Integer type, ShopOrder shopOrder) {
        try {
            ...
            String isStepOrder = extraMap.get(TradeConstants.IS_STEP_ORDER);
            String stepOrderStatus = extraMap.get(TradeConstants.STEP_ORDER_STATUS);
            Boolean isPreOrder = Boolean.FALSE;//是否是预售订单
            if (!StringUtils.isEmpty(isStepOrder) && Objects.equals(isStepOrder, "true")) {
            ...
            if (Objects.equals(type, 1)) {
                ...
                if (!response.isSuccess()) {
                    ...
                    dashBoardShipmentDTO.setBillNo(shopOrder.getOutId());
                    //订单类型
                    dashBoardShipmentDTO.setBillType(isPreOrder?"预售":"普通");
                    //出货店铺代码（提醒信息的店铺）
     ...
     * @return 枚举类型
     */
    public Integer error2Type(String error) {
        switch (error) {
