...
public class AdminOpenShops {
    ...
    //特殊接口，非通用
    @ApiOperation("根据门店和账套查询门店")
    @RequestMapping(value = "/searchByShopNameAndCompanyCode", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public Paging<OpenShop> searchByShopNameAndCompanyCode (String shopName, String companyCode, Integer pageNum, Integer pageSize) {
        //过滤掉oxo和银泰的店铺
        List<String> notInChannel = Lists.newArrayList("vipoxo", "yintai");
        //有规则的店铺
        Set<Long> allConfigShopIds = warehouseRulesClient.findAllConfigShopIds();
        Paging<OpenShop> openShopPaging = openShopExtDao.searchByShopNameAndCompanyCode(notInChannel, shopName, companyCode, pageNum, pageSize, allConfigShopIds);
        return openShopPaging;
    }
    ...
    private void editExtraInfo (OpenShop openShop, Map<String, String> map) {
        ...
        //是否管控，分组、发货限制及安全库存
        map.put(TradeConstants.MANUAL_DISPATCH_RULE_CONTROL, jsonMap.getOrDefault(TradeConstants.MANUAL_DISPATCH_RULE_CONTROL, "false"));
