...
public class MiddleConfiguration extends WebMvcConfigurerAdapter {
    ...
    @Bean("yintaiEventBus")
    public EventBus yintaiEventBus() {
        return new AsyncEventBus(
                new ThreadPoolExecutor(5, 10, 0, TimeUnit.MILLISECONDS,
                        new ArrayBlockingQueue<>(100000), (new ThreadFactoryBuilder()).setNameFormat("yintai-event-bus-%d").build(),
                        (r, executor) -> log.error("yintai event {} is rejected", r)));
    }
