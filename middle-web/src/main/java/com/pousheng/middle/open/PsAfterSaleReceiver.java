...
public class PsAfterSaleReceiver extends DefaultAfterSaleReceiver {
    ...
    protected List<Refund> fillSkuInfo(ShopOrder shopOrder, Refund refund, List<SkuOfRefund> skuOfRefundList) {
        ...
        if ((Objects.equals(shopOrder.getOutFrom(), MiddleChannel.TAOBAO.getValue())
          || Objects.equals(shopOrder.getOutFrom(), MiddleChannel.YOUZAN.getValue()))
    ...
    @Override
    protected Response<Boolean> updateRefund(Refund refund, OpenClientAfterSale afterSale) {
    ...
    @Override
    protected Response<Long> createRefund(Refund refund, ShopOrder shopOrder) {
        Response<Long> createResponse=super.createRefund(refund, shopOrder);
        if(createResponse.isSuccess()){
            //唯品会的订单自动审核派发处理逻辑
            if(Objects.equals(shopOrder.getOutFrom(),MiddleChannel.VIPOXO.getValue())){
                refundPostProcessor.postProcessorAfterCreated(createResponse.getResult());
            }
        }
