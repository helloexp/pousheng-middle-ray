...
public class Refunds {
    ...
    @org.springframework.beans.factory.annotation.Value("${logging.path}")
    private String refundTemplePath;
        ...
        try{
            if (Objects.equals(refund.getRefundType(),MiddleRefundType.LOST_ORDER_RE_SHIPMENT.value())){
                ...
                if(Objects.equals(refund.getRefundType(),MiddleRefundType.AFTER_SALES_RETURN) || Objects.equals(refund.getRefundType(),MiddleRefundType.AFTER_SALES_CHANGE)){
                    Long editshipmentid = editSubmitRefundInfo.getShipmentId();
                    Shipment currentshipment = shipmentReadLogic.findShipmentById(editshipmentid);
                    String refundCompanyIsNot325 = checkShopOrWarehouse(currentshipment,refund,Long.valueOf(editSubmitRefundInfo.getReturnStockid()));
     ...
     * @return
     */
    @RequestMapping(value = "/api/refundTemplates/onlyMoneyAndGoods/import", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public Response<String> importRefunds(MultipartFile file){
       try {
           Set<String> allowExts = Sets.newHashSet("xlsx");

           String fileExtName = getExtRefundName(file);
           if (!allowExts.contains(fileExtName)) {
               throw new JsonResponseException("文件格式不正确, 请上传正确的文件");
           }          
          
           //解析文件
           List<MiddleRefundInfoExt> refundext = HandlerFileUtil.getInstance().handlerExcelRefundInfoExt(file.getInputStream());
           if (log.isDebugEnabled()){
               log.debug("refundLogic importMiddleRefund,refundTemplateInfos {}",refundext);
           }
           String batchCode = "";
           if(!CollectionUtils.isEmpty(refundext)){               
           //生成导入批次唯一码标识
           batchCode = randomCode();
           List<Refundtemplates>  reftempList = Lists.newArrayList();
           Refundtemplates reftemp = null;
           for(MiddleRefundInfoExt ext : refundext){
               reftemp = new Refundtemplates();
               reftemp.setBatchCode(batchCode);
               reftemp.setSaleCode(ext.getSaleCode());
               reftemp.setRefundType(Integer.parseInt(ext.getRefundType()));
               reftemp.setShipCode(ext.getShipCode());
               reftemp.setSkuCode(ext.getSkuCode());
               reftemp.setNum(Integer.parseInt(ext.getNum()));
               reftemp.setCreatedAt(new Date());
               reftemp.setUpdatedAt(new Date());
           }
               reftempList.add(reftemp);
               boolean createtemplates = refundWriteLogic.createsRefundTemplates(reftempList);
               if(!createtemplates){
                   throw new JsonResponseException("售后单文件导入失败");  
               }
           }
           return Response.ok(batchCode);           
       } catch (Exception e) {
        log.error("create refund onlyMoneyAndGoods failed,caused by {}", Throwables.getStackTraceAsString(e));
           return Response.fail(e.getMessage());
       }       
    }

    /**
     * 通过batchCode查询售后单模板数据list
     * @param batchCode
     * @return
     */
    @RequestMapping(value = "/api/refundTemplates/{code}/getList", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public List<Refundtemplates> findRefTemplatesList(@PathVariable("code") @PermissionCheckParam String batchCode){
        if(log.isDebugEnabled()){
            log.debug("QUERY-REFUNDTEMPLATES-LIST-START param: batchcode [{}]", batchCode);
        }
        return refundReadLogic.getRefTemplates(batchCode);
    }

    /**
     * 根据refundtemplates的id更新status为1：创建售后单成功
     * @param id
     */
    @RequestMapping(value = "/api/refund/{id}/CreatedStatus",method = RequestMethod.PUT)
    public void syncRefundTempStatus(@PathVariable("id") @PermissionCheckParam Long id){
        if(log.isDebugEnabled()){
            log.debug("UPDATE-REFUNDTEMPLATES-STATUS-START param: ID [{}]", id);
        }
        refundWriteLogic.updateRefTemplatesStatus(id);
    }
    
    @GetMapping("/api/export/refund/moneyAndGoods/template")
    public ResponseEntity<byte[]> exportRefundTemplate(HttpServletResponse response, HttpServletRequest request, @RequestParam(required = false, defaultValue = "excel") String fileType){
        String fileName = refundTemplePath;
        fileName += "/仅退货退款售后单模板";
    ...
    private String randomCode(){
        ...
        //时间戳+3位随机数字
        String rdCode = System.currentTimeMillis()+""+rd;         
        return rdCode;        
    }

    private ResponseEntity<byte[]> downLoadsFile(String fileName) {
...
//        shipmentWiteLogic.updateExtra(shipment.getId(), shipmentExtraMap);
    ...
    public void exchangeToRefund(@PathVariable("id")@OperationLogParam Long id){
        //TODO 判断控制 渠道 三方  是否拉取换货
        if(log.isDebugEnabled()){
            log.debug("API-REFUND-CANCELREFUNDFORCHANGE-START param: id [{}]", id);
        }
        Refund originRefund = refundReadLogic.findRefundById(id);
        ...
        if (refundReadLogic.isExchangeTorefund(originRefund)){
            //如果允许换转退则修改状态，退货改换货需要判断一下是否存在有库存没有释放的情况
            refundWriteLogic.releaseRejectShipmentOccupyStock(id);
            Refund refund = refundReadLogic.findRefundById(id);
            //如果允许取消，则将占库发货单取消即可
            refundWriteLogic.cancelAfterSaleOccupyShipments(id);
            ...
            for (RefundItem refundItem : refundItems) {
                //更新发货单中已经售后的商品的数量
                refundWriteLogic.updateShipmentItemRefundQuantity(refundItem.getSkuCode(),
                        StringUtils.isEmpty(refundItem.getFinalRefundQuantity())?refundItem.getFinalRefundQuantity():refundItem.getApplyQuantity(), shipmentItems);
            }

            refundWriteLogic.updateStatusLocking(refund,MiddleOrderEvent.AFTER_SALE_EXCHANGE_TO_RETURN.toOrderOperation());
            Flow flow = flowPicker.pickAfterSales();
            ...
            refundWriteLogic.update(refund);
            //TODO
            //点击“换转退”后，售后换货单变更类型“退货退款”状态为“待退款”，退货数量以实际退货数 量为准，退款金额也以实际退款金额为准
    ...
    private String getisNot325CompanyCode(Refund refund, Map<String,String> refundExtraMap){
        ...
        if(refundExtraMap.containsKey(TradeConstants.REFUND_EXTRA_INFO)){
            ...
            // 发货单id
            String shipmentid = refundExtra.getShipmentId();
            // 退货店仓id
    ...
    private String checkShopOrWarehouse(Shipment shipment,Refund refund,Long warehouseid){
            ...
            Long deliverid = shipmentExtra.getWarehouseId();
            String companyCode = null;
            if(Objects.equals(shipment.getShipWay(), TradeConstants.MPOS_SHOP_DELIVER)){
                Response<OpenShop> openshopRes = openShopReadService.findById(deliverid);
                if(openshopRes.isSuccess() && openshopRes.getResult() != null){
                    Map<String,String> shopExtraMap = openshopRes.getResult().getExtra();
                    if(!Objects.isNull(shopExtraMap) && shopExtraMap.containsKey(TradeConstants.HK_COMPANY_CODE)){
                ...
                WarehouseDTO checkWarehousedto = findWarehouseById(deliverid);
                if(checkWarehousedto != null && Objects.equals(companyCode,TradeConstants.BAO_WEI_COMPANY_ID.toString())){
                    refundCompanyIsNot325 = addIsNot325Company(refund, warehouseid);
                }
            }
        return refundCompanyIsNot325;
    ...
    private String getExtRefundName(MultipartFile file) {
