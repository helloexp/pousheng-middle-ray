...
import io.terminus.open.client.item.dto.StockResult;
...
public class StockPusherLogic {
    ...
    public void sendToYj(Long shopId, List<YjStockInfo> yunjuStockInfoList,
                         ArrayListMultimap<String, YjStockInfo> skuCode2YjStockInfos) {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            if (!yunjuStockInfoList.isEmpty()) {
                ...
                if (stockPusherCacheEnable && resp.isSuccess()) {
                    yunjuStockInfoList.forEach(yjStockInfo -> {
                        List<YjStockInfo> yjStockInfoList = skuCode2YjStockInfos.get(yjStockInfo.getBarCode());
                        if (CollectionUtils.isEmpty(yjStockInfoList)) {
                            write2Redis(yjStockInfo);
                        } else {
                            yjStockInfoList.forEach(this::write2Redis);
                        }
                    });
                }
                //写入推送日志
                ...
                yunjuStockInfoList.forEach(yjStockInfo -> {
                    List<YjStockInfo> yjStockInfoList = skuCode2YjStockInfos.get(yjStockInfo.getBarCode());
                    if (CollectionUtils.isEmpty(yjStockInfoList)) {
                        StockPushLog stockPushLog = genStockPushLog(
                                yjStockInfo, traceId, shopId, shopName, shopCode, status, cause, curDate);
                        stockPushLogs.add(stockPushLog);
                    } else {
                        yjStockInfoList.forEach(it -> {
                            StockPushLog stockPushLog = genStockPushLog(
                                    it, traceId, shopId, shopName, shopCode, status, cause, curDate);
                            stockPushLogs.add(stockPushLog);
                        });
                    }
                });
    ...
    private StockPushLog genStockPushLog(YjStockInfo yjStockInfo, String traceId, Long shopId, String shopName,
                                         String shopCode, Integer status, String cause, Date syncAt) {
        WarehouseDTO warehouse =
                warehouseCacher.findByOutCodeAndBizId(yjStockInfo.getWarehouseCode(), yjStockInfo.getCompanyCode());
    ...
    private void write2Redis(YjStockInfo yjStockInfo) {
        WarehouseDTO warehouse =
                warehouseCacher.findByOutCodeAndBizId(yjStockInfo.getWarehouseCode(), yjStockInfo.getCompanyCode());
    ...
    public void prallelUpdateStock(ItemMapping itemMapping, Long stock, OpenShop openShop) {
        String originSkuCode = null;
        if (MiddleChannel.from(openShop.getChannel()) == MiddleChannel.YINTAI) {
            originSkuCode = itemMapping.getSkuCode();
            itemMapping.setSkuCode(YintaiOutIdBuilder.getOutId(itemMapping.getSkuCode(), /*这里是spuCode*/itemMapping.getItemName()));//skuCode+spuCode
        }
        if (log.isDebugEnabled()) {
        ...
        Response<StockResult> rP = null;
        if (extra.get("isYunDing") != null && Objects.equals(extra.get("isYunDing"), "true")) {
            Response<Boolean> response = jdYunDingSyncStockLogic.syncJdYundingStock(itemMapping, Math.toIntExact(stock));
            if (!response.isSuccess()) {
                log.error("failed to push isYunDing stock of sku(skuCode={}) to shop(id={}), error code{}",
                        itemMapping.getSkuCode(), itemMapping.getOpenShopId(), response.getError());
                rP = Response.ok(StockResult.builder().success(Boolean.FALSE).execute(Boolean.TRUE).stock(stock.intValue()).build());
                rP.setError(response.getError());
            } else {
                rP = Response.ok(StockResult.builder().success(Boolean.TRUE).execute(Boolean.TRUE).stock(stock.intValue()).build());
            }

        } else {
            rP = itemServiceCenter.updateSkuStockByBill(openShop.getId(), itemMapping, stock.intValue());
            if (!rP.isSuccess()) {
        ...
        if (MiddleChannel.from(openShop.getChannel()) == MiddleChannel.YINTAI) {
            //还原
            itemMapping.setSkuCode(originSkuCode);
            shopId = itemMapping.getOpenShopId();//店柜
            createAndPushLogsForYintai(stockPushLogs, itemMapping, openShop, stock, rP);
        } else {
            createAndPushLogs(stockPushLogs, itemMapping.getSkuCode(), openShop.getId(), itemMapping.getChannelSkuId(), stock, rP.isSuccess(), rP.getError());
            pushLogs(stockPushLogs);
        }
        //如果推送成功则将本次推送记录写入缓存,第三方店铺key值写入channelSkuId
        if (stockPusherCacheEnable && rP.isSuccess()) {
            stockPushCacher.addToRedis(StockPushCacher.ORG_TYPE_SHOP, shopId.toString(), itemMapping.getChannelSkuId(), stock.intValue());
        }
    }
        ...
        CompletableFuture.runAsync(() -> {
            ...
            for (Long shopId : shopSkuStockMap.keySet()) {
                try {
                    ...
                    List<StockPushLog> stockPushLogs = Lists.newArrayList();
                    //库存日志推送
                    paranaSkuStocks.forEach(stock -> createAndPushLogs(stockPushLogs, stock.getSkuCode(), shopId, null, stock.getStock().longValue(), r.isSuccess(), r.getError()));
                    pushLogs(stockPushLogs);
    ...
    private Long calculateWarehouseStockForShop(List<AvailableInventoryDTO> dtos, ShopStockRuleDto shopStockRuleDto, String skuCode) {
        ...
        if (isSubtractWaitHandle == null) {
            throw new ServiceException("库存推送规则未设置");
        }
        if (channelStock < 0L) {
        ...
        if (null != shopRule.getSafeStock()) {
            log.info("log info1 channelStock({}),shareStock({})", channelStock, shareStock);
        ...
        if (isSubtractWaitHandle && CollectionUtils.isEmpty(shopStockRuleDto.getWarehouseRule())) {
            ...
            log.info("log info2 channelStock({}),shareStock({})", channelStock, shareStock);
            //按照设定的比例确定推送数量
            stock = Math.max(0,
                    channelStock
                            + shareStock * shopRule.getRatio() / 100
     ...
     */
    //todo 缓存优化
    public List<OpenShop> getShopByChannel(String channel) {
        Response<List<OpenShop>> resp = openShopReadService.findByChannel(channel);
        if (!resp.isSuccess()) {
    ...
    private void createAndPushLogsForYintai(List<StockPushLog> logs,
                                   ItemMapping itemMapping,
                                   OpenShop openShop,
                                   Long stock,
                                   Response<StockResult> rP) {
        try {
            StockResult result = rP.getResult();
            StockPushLog stockPushLog = StockPushLog.builder()
                    .shopId(openShop.getId())
                    .shopName(itemMapping.getOpenShopName())
                    .outId(itemMapping.getOpenShopId().toString())
                    .skuCode(itemMapping.getSkuCode())
                    .channelSkuId(itemMapping.getChannelSkuId())
                    .quantity(stock)
                    .status(rP.isSuccess() ? 1 : 2)
                    //调整库存数 这里借用字段吧，大表增加字段代价太大
                    .requestNo(result != null ? result.getAdjustStock() != null ? result.getAdjustStock().toString() : null : null)
                    .cause(rP.getError()).syncAt(DateTime.now())
                    .build();
