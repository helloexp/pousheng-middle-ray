...
import com.pousheng.middle.open.mpos.dto.MposShipmentResponse;
...
public class SyncMposShipmentLogic{
    ...
    /**
     * 根据Middle的shipment, 拉取Mpos发货单
     * @param shipment
     * @return
     */
    public MposShipmentExtra syncMposShipmentInfo(Shipment shipment) {
    	try {
			log.info("sync mpos shipment start, shipmentId:{}", shipment.getId());
			Map<String, Object> param = Maps.newHashMap();
			param.put("outerShipmentId", String.valueOf(shipment.getId()));
			MposShipmentResponse resp = mapper.fromJson(syncMposApi.syncMposShipmentInfo(param), MposShipmentResponse.class);
			if (!resp.getSuccess()) {
				log.info("find mpos shipment failed, shipmentId:{}, cause:{}", shipment.getId(), resp.getError());
				throw new ServiceException(resp.getError());
			}
			log.info("sync mpos shipment end, shipmentId:{}", shipment.getId());
			return resp.getResult();
		} catch (Exception var1) {
			log.info("sync mpos shipment failed, shipmentId:{}, cause:{}", shipment.getId(), Throwables.getStackTraceAsString(var1));
			return new MposShipmentExtra();
		}
    }
}
