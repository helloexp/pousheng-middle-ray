...
public class SyncShipmentPosLogic {
            ...
            hkShipmentPosItem
                    .setBalaprice(new BigDecimal(shipmentItem.getCleanPrice() == null ? 0 + balapriceOfIntegralPart
                            : shipmentItem.getCleanPrice() + balapriceOfIntegralPart)
                                    .divide(new BigDecimal(100), 2, RoundingMode.HALF_DOWN).toString());
            //积分商品 积分
            hkShipmentPosItem.setDischargeintegral(integral.toString());
            //积分商品抵现金额
            hkShipmentPosItem.setDischargeintegralamount(new BigDecimal(balapriceOfIntegralPart).divide(new BigDecimal(100), 2, RoundingMode.HALF_DOWN).toString());
    ...
    private HkShipmentPosInfo makeHkShipmentPosInfo(ShipmentDetail shipmentDetail, String shipmentWay) {
        ...
        //减去优惠金额
        totalPrice = totalPrice.add(new BigDecimal(shipmentExtra.getShipmentShipFee() == null ? 0 : shipmentExtra.getShipmentShipFee())).subtract(new BigDecimal(shipmentExtra.getShipmentShipDiscountFee() == null ? 0 : shipmentExtra.getShipmentShipDiscountFee()));
        posInfo.setPayamountbakup(totalPrice.divide(new BigDecimal(100), 2, RoundingMode.HALF_DOWN).toString()); //线上实付金额

        BigDecimal realShipmentFee=new BigDecimal(shipmentExtra.getShipmentShipFee() == null ? 0 : shipmentExtra.getShipmentShipFee()).subtract(new BigDecimal(shipmentExtra.getShipmentShipFee() == null ? 0 : shipmentExtra.getShipmentShipDiscountFee()));
        //传恒康运费=应支付运费减去优惠的运费
        posInfo.setExpresscost(realShipmentFee.divide(new BigDecimal(100), 2, RoundingMode.HALF_DOWN).toString());//邮费成本
