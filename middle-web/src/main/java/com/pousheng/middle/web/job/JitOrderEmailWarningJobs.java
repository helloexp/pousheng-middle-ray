...
@RestController
public class JitOrderEmailWarningJobs {

    private static final int BATCH_SIZE = 100;

    private static final String FILE_NAME_TEMPLATE="JIT订单-{0}.xls";
    ...
    private static final String EMAIL_TITLE="待处理待JIT订单";
     ...
     * 收件人
     */
    @Value("${jit.order.warning.email.receives}")
     ...
     * 默认为
     */
    @Scheduled(cron ="${jit.order.warning.job.cron}")
    public void run(){

        if(!hostLeader.isLeader()) {
            log.info("jit email warning job current leader is:{}, skip ", hostLeader.currentLeaderId());
            return;
        }
        log.info("jit the email warning job of failed order start");
        try{
            ...
            log.error("the jit email warning job error.{}",e);
        }
        log.info("the jit email warning job of failed order end");
    }
    ...
    private List<String> queryOrders(){
        ...
        do{
            ...
            orderCriteria.setType(MiddleOrderType.JIT.getValue());
    ...
    private List<String> queryShipments(){
        ...
        do{
            ...
            criteria.setShopId(Long.valueOf(shopId));
