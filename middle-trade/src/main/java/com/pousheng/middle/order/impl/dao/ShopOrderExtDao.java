...
public class ShopOrderExtDao extends MyBatisDao<ShopOrderExt> {
     ...
     * @return
     */
    public Paging<ShopOrder> findByecpOrderStatus(Integer offset, Integer limit, String dtStart,String dtEnd){
    	
        // get total count
        Long total = sqlSession.selectOne(sqlId("findByecpOrderStatusCount"), ImmutableMap.of("dtStart", dtStart,"dtEnd", dtEnd));
        if (total <= 0){
            return new Paging<ShopOrder>(0L, Collections.<ShopOrder>emptyList());
        }
        
        // get data
        List<ShopOrder> datas = getSqlSession().selectList(sqlId("findByecpOrderStatus"),ImmutableMap.of("offset",offset,"limit",limit,"dtStart", dtStart,"dtEnd", dtEnd));
