...
public class WarehouseRulesClient {
    ...
    public Response<List<WarehouseDTO>> findWarehousesByShopId(Long openShopId) {
        try {
            if (null == openShopId) {
                return Response.ok(Lists.newArrayList());
            }

            return Response.ok((List< WarehouseDTO >) inventoryBaseClient.get("api/inventory/rule/findWarehousesByShopId/" + openShopId,
                    null, null, Maps.newHashMap(), WarehouseDTO.class, true));
        } catch (Exception e) {
            log.error("failed to find warehouseIds By ShopId, openShopId:{}, cause:{}", openShopId, Throwables.getStackTraceAsString(e));
        }
    ...
    public String batch(RuleItemBatchRequest request) {
        String pathDelete = "/api/inventory/rule/rule-item/batch/delete";
        String pathCreate = "/api/inventory/rule/rule-item/batch/create";
        String result;
        if (request.getIsDelete()) {
            result = (String)inventoryBaseClient.postJson(pathDelete, JsonMapper.nonEmptyMapper().toJson(request), String.class);
        } else {
            result = (String)inventoryBaseClient.postJson(pathCreate, JsonMapper.nonEmptyMapper().toJson(request), String.class);
        }
        return result;
    }
}
