...
public class RefundReadLogic {
    ...
    @Autowired
    private RefundTemplatesReadService refundTemplatesReadService;
    ...
    public List<Refundtemplates> getRefTemplates(String batchCode){
        if(!StringUtils.hasText(batchCode)){
            return null;
        }
        Response<List<Refundtemplates>> refTempresponse = refundTemplatesReadService.findByBatchCode(batchCode);
        if(!refTempresponse.isSuccess()){
            log.error("fail to query refundTemplates by batchCode:{}, cause:{}",batchCode, refTempresponse.getError());
            throw new JsonResponseException(refTempresponse.getError());
        }
        List<Refundtemplates> refTempList = refTempresponse.getResult();
        return refTempList;
    }
