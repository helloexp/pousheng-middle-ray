...
public class MiddleOrderReadServiceImpl implements MiddleOrderReadService {
    ...
    @Override
    public Response<Paging<ShopOrder>> findByecpOrderStatus(Integer offset, Integer limit,String dtStart,String dtEnd) {
        try {
            return Response.ok(shopOrderExtDao.findByecpOrderStatus(offset,limit, dtStart,dtEnd));
        }catch (Exception e){
            log.error("failed to find shop orders by ecp order stauts,dtStart:{},dtEnd:{}",dtStart,dtEnd,
                Throwables.getStackTraceAsString(e));
            return Response.fail("failed.to.find.shop.orders");
        }
    }
