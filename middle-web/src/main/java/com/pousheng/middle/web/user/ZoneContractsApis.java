...
public class ZoneContractsApis {
    ...
    public Map<String, Long> editZoneContract(@RequestBody @LogMeContext ZoneContract zoneContract) {
        ...
        if (Objects.isNull(zoneContract.getId())) {
            ...
            if (!response.isSuccess()) {
                ...
                throw new JsonResponseException("zoneName.contract.create.fail");
            }
            ...
            if (!response.isSuccess()) {
                ...
                throw new JsonResponseException("zoneName.contract.update.fail");
            }
    ...
    public Response<Boolean> del(@PathVariable("id") @LogMeId Long id) {
        ...
        if (!response.isSuccess()) {
            ...
            throw new JsonResponseException("zoneName.contract.delete.fail");
        }
        if(log.isDebugEnabled()){
    ...
    private void validateParm(ZoneContract zoneContract) {
        ...
        if (Arguments.isEmpty(zoneContract.getZoneId())) {
            throw new JsonResponseException("zoneName.id.empty");
        }
        if (Arguments.isEmpty(zoneContract.getZoneName())) {
            throw new JsonResponseException("zoneName.name.empty");
        }

        if (StringUtils.isEmpty(zoneContract.getName())) {
            throw new JsonResponseException("zoneName.contract.name.empty");

        }
        if (StringUtils.isEmpty(zoneContract.getEmail())) {
            throw new JsonResponseException("zoneName.contract.email.empty");
        }
        if (Objects.isNull(zoneContract.getGroup())) {
            throw new JsonResponseException("zoneName.contract.group.empty");
        }

        if (!ZoneGroupEnum.contains(zoneContract.getGroup())) {
            throw new JsonResponseException("zoneName.contract.group.illegal");
        }
    }
}
