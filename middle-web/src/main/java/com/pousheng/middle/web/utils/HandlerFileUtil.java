...
import com.pousheng.middle.order.dto.MiddleRefundInfoExt;
...
public class HandlerFileUtil<T> {
    ...
    public List<MiddleRefundInfoExt> handlerExcelRefundInfoExt(InputStream insr) throws IOException {
        List<MiddleRefundInfoExt> refundInfoExtS = Lists.newArrayList();
        long startTime = System.currentTimeMillis();
        BufferedInputStream bufferedInputStream = new BufferedInputStream(insr);
        try {
            //存在serialVersionUID，需要去掉
            Field[] fields = MiddleRefundInfoExt.class.getDeclaredFields();
            List<String[]> list = ExcelCovertCsvReader
                    .readerExcelAt(
                            bufferedInputStream, fields.length - 1, 501);
            for (Integer i = 1; i < list.size(); i++) {
                ...
                MiddleRefundInfoExt middleRefundInfoExt = makeRefundInfoExt(list.get(i), fields);
                if (Strings.isNullOrEmpty(middleRefundInfoExt.getSaleCode())) {
                    throw new ServiceException("第"+i+"行：交易单号不能为空");
                }
                if (Strings.isNullOrEmpty(middleRefundInfoExt.getRefundType())){
                    throw new ServiceException("第"+i+"行：售后类型不能为空");
                } else if(!Objects.equals(middleRefundInfoExt.getRefundType(),"2")){
                    continue;
                }
                if (Strings.isNullOrEmpty(middleRefundInfoExt.getShipCode())) {
                    throw new ServiceException("第"+i+"行：发货单号不能为空");
                }
                if (Strings.isNullOrEmpty(middleRefundInfoExt.getSkuCode())) {
                    throw new ServiceException("第"+i+"行：商品条码不能为空");
                }
                if (!isPositiveNumber(middleRefundInfoExt.getNum())) {
                    throw new ServiceException("第"+i+"行：商品数量必须为正整数");
                }

                refundInfoExtS.add(middleRefundInfoExt);
            }
            long endTime = System.currentTimeMillis();
            log.info("refund moneyAndGoods import excel , date:{}", endTime - startTime);
        } catch (ServiceException | NullPointerException e) {
            throw new ServiceException(e.getMessage());
        } catch (Exception e) {
            log.error("refund moneyAndGoods import excel fail, cause:{}", Throwables.getStackTraceAsString(e));
            throw new ServiceException("refund.moneyAndGoods.import.excel.fail");
        } finally {
            bufferedInputStream.close();
        }
        return refundInfoExtS;
    }
     ...
     * 解析excel
     */
    private MiddleRefundInfoExt makeRefundInfoExt(String[] data, Field[] fields) {

        try {
            MiddleRefundInfoExt middleRefundInfoExt = new MiddleRefundInfoExt();
            for (Integer j = 1; j < fields.length; j++) {
                //把excel数据导入到bean类
                String name = fields[j].getName();
                name = name.substring(0, 1).toUpperCase() + name.substring(1);
                Method method = middleRefundInfoExt.getClass().getMethod("set" + name, fields[j].getType());
                method.invoke(middleRefundInfoExt, data[j - 1] != "" ? data[j - 1] : null);
            }
            return middleRefundInfoExt;
        } catch (Exception e) {
            log.error("refund goods import excel fail, cause:{}", Throwables.getStackTraceAsString(e));
            throw new ServiceException("refund.goods.import.excel.fail");
        }
    }
