...
public class CompensateBizLogic {
     ...
     * 重新发送消息
     */
    public void reSendBiz(PoushengCompensateBiz compensateBiz, String topic) {
        if (log.isDebugEnabled()) {
            log.debug("CompensateBizLogic resend compensateBiz {}", compensateBiz);
        }
        ...
        log.info("create biz success and send mq topic: {} message: {}", topic, compensateBiz.getId());
        rocketMqProducerService.asyncSendOrderly(topic, JsonMapper.nonEmptyMapper().toJson(compensateBiz.getId()), compensateBiz.getId(), queueSize);
    }
    ...
    public void consumeMqMessage(String message) {
        ...
        try {
            ...
            log.info("Current biz(id:{}, bizId:{}, context:{})'s status is {}", poushengCompensateBiz.getId(),
                poushengCompensateBiz.getBizId(), poushengCompensateBiz.getContext(), poushengCompensateBiz.getStatus());
            ...
            if (!rU.isSuccess()) {
                ...
                // 捞到内存再判断一次
                Response<PoushengCompensateBiz> found = poushengCompensateBizReadService.findById(poushengCompensateBiz.getId());
                if (!found.isSuccess()) {
                    log.error("failed to find compensate biz by id: {}, cause: {}", poushengCompensateBiz.getBizId());
                    return;
                }

                PoushengCompensateBiz exist = found.getResult();
                if (!Objects.equals(exist.getStatus(), PoushengCompensateBizStatus.WAIT_HANDLE.name())) {
                    log.warn("expect status WAIT_HANDLE, current biz type {} mismatch", exist.getStatus());
                    return;
                }
                // 直接更新
                PoushengCompensateBiz toUpdate = new PoushengCompensateBiz();
                toUpdate.setId(exist.getId());
                toUpdate.setStatus(PoushengCompensateBizStatus.PROCESSING.name());
                Response<Boolean> r = poushengCompensateBizWriteService.update(toUpdate);
                if (!r.isSuccess()) {
                    log.error("failed to update compensate(id={}) status to processing, cause: {}", exist.getId(), r.getError());
                    return;
                }
            }
            //业务处理
        ...
        } catch (NoTryException e) {
            log.error("process pousheng biz failed,id is {},bizType is {},caused by {}", poushengCompensateBiz.getId(), poushengCompensateBiz.getBizType(), e.getMessage());
            ...
            poushengCompensateBizWriteService.updateLastFailedReason(poushengCompensateBiz.getId(), e.getMessage(), 9);
