...
public class AdminOrderWriter {
    ...
    private final Integer MAX_EXCEL_ROW_BATCH_UPDATE = 500;
//    @Autowired
//    private MessageSource messageSource;
    ...
    public void addCustomerServiceNote(@PathVariable("id") @LogMeContext Long id, @RequestParam("customerSerivceNote") @LogMeContext String customerSerivceNote) {
        ...
        autoCreateRejectOrderHandler.cancelOxoOrder(id);
    }
    ...
    public Response<Boolean> customerServiceCancelSkuOrder(@PathVariable("id") @OperationLogParam Long id, @RequestParam String skuOrderCancelReason) {
        ...
        SkuOrder skuOrder = getSkuOrderForCancel(id);
        if (skuOrder.getShipping() > 0 || skuOrder.getShipped() > 0) {
            throw new JsonResponseException("sku.order.exist.part.shipping");
        }

        Response<Boolean> r = doCancel(skuOrder, skuOrderCancelReason, MiddleOrderStatus.CANCEL);
        if (log.isDebugEnabled()) {
    ...
    /**
     * 中台客服部分取消sku订单(部分发货，且没有代发货商品)
     *
     * @param id 店铺订单id
     * @return
     */
    @RequestMapping(value = "/api/order/sku/{id}/customer/service/cancel/partial", method = RequestMethod.PUT)
    @OperationLogType("中台客服取消子订单")
    public Response<Boolean> customerServicePartialCancelSkuOrder(@PathVariable("id") @OperationLogParam Long id, @RequestParam String skuOrderCancelReason) {
        if (log.isDebugEnabled()) {
            log.debug("API-CUSTOMERSERVICEPARTIALCANCELSKUORDER-START param: id [{}] skuOrderCancelReason [{}]", id, skuOrderCancelReason);
        }

        SkuOrder skuOrder = getSkuOrderForCancel(id);
        MiddleOrderStatus targetStatus = MiddleOrderStatus.CANCEL;
        // 部分取消逻辑：部分发货，待处理数量不为0
        if (skuOrder.getShipping() > 0 || skuOrder.getShipped() > 0) {
            // 1. 待处理数量清零
            // 如果有部分商品未处理，取消该订单直接将订单置为处理完成即可（代发货状态）
            int withHold = MoreObjects.firstNonNull(skuOrder.getWithHold(), 0);
            int quantity = MoreObjects.firstNonNull(skuOrder.getQuantity(), 0);
            if (withHold > 0 && quantity > withHold) {
                // extra 中的待处理数量
                if (skuOrder.getExtra().containsKey(TradeConstants.WAIT_HANDLE_NUMBER)) {
                    Map<String, String> extra = skuOrder.getExtra();
                    extra.put(TradeConstants.WAIT_HANDLE_NUMBER, "0");
                    // 后续流程会更新 extra，这里不处理
                    skuOrder.setExtra(extra);
                }
                // sku 待处理字段
                SkuOrder toUpdate = new SkuOrder();
                toUpdate.setId(skuOrder.getId());
                toUpdate.setWithHold(0);
                Response<Boolean> r = middleOrderWriteService.updateSkuOrder(toUpdate);
                if (!r.isSuccess()) {
                    throw new JsonResponseException("update.sku.order.failed");
                }

                // 获取最小状态，默认代发货
                List<Shipment> shipments = shipmentReadLogic.findByOrderIdAndSkuCode(skuOrder.getOrderId(), skuOrder.getSkuCode(), null);
                Integer minStatus = shipments.stream()
                ...
                if (minStatus == null) {
                    ...
                    targetStatus = MiddleOrderStatus.SHIPPED;
                }
            }
        }
    ...
    private SkuOrder getSkuOrderForCancel(Long id) {
        SkuOrder skuOrder = (SkuOrder) orderReadLogic.findOrder(id, OrderLevel.SKU);
    ...
    private Response<Boolean> doCancel(SkuOrder skuOrder, String skuOrderCancelReason, MiddleOrderStatus targetStatus) {
        // 验证是否正在创建发货单
        ...
        // 尝试更新订单状态
        Response<Boolean> r = orderWriteService.skuOrderStatusChanged(skuOrder.getId(), MiddleOrderStatus.WAIT_HANDLE.getValue(), targetStatus.getValue());
        if (!r.isSuccess()) {
            log.error("sku order cancel failed,skuOrderId is {},caused by{}", skuOrder.getId(), r.getError());
            throw new JsonResponseException("cancel.sku.order.failed");
        }
        //释放mpos占用的库存
	...
	/**
	 * XXX RAY POUS923 2019.06.14 下載批量修改条码模板
	 * 
	 * @param response
	 * @param request
	 * @return
	...
	public ResponseEntity<byte[]> getBatchUpdateSkuTemplate(HttpServletResponse response,
		...
		fields.add(new String[] { "订单来源", "外部订单号", "原货品条码", "修改后货品条码" });
	...
	/**
	 * XXX RAY POUS923 2019.04.24: 批量修改待處理的商品條碼
	 * 
	 * @param file
	 * @param request
	 * @return 更新結果
	...
	@RequestMapping(value = "/api/sku/order/batchUpdate/sku/file", method = RequestMethod.PUT,
			produces = MediaType.APPLICATION_JSON_VALUE)
	public Response<Map<String, Object>> batchUpdateSkuCode(MultipartFile file,
			HttpServletRequest request) {
		if (file == null) {
		...
		try {
			...
			List<String[]> dataList = ExcelCovertCsvReader.readerExcelAt(inputStream, 4,
					MAX_EXCEL_ROW_BATCH_UPDATE);
			// 2019.06.27 上傳錯誤格式，拋出例外
			if (CollectionUtils.isEmpty(dataList)) {
				return Response.fail("upload.file.type.error");
			}
			dataList.remove(0); // 移除EXCEL表頭
			// 2019.06.25 過濾空數據
			dataList = dataList.stream()
					.filter(arr -> !(StringUtils.trimToEmpty(arr[0]).isEmpty()
							&& StringUtils.trimToEmpty(arr[1]).isEmpty()
							&& StringUtils.trimToEmpty(arr[2]).isEmpty()
							&& StringUtils.trimToEmpty(arr[3]).isEmpty())).collect(Collectors.toList());
			if (CollectionUtils.isEmpty(dataList)) {
				return Response.fail("export.data.empty");
			}
			if (MAX_EXCEL_ROW_BATCH_UPDATE < dataList.size()) { // 超過上傳最大值
				throw new InvalidException("order.import.excel.more.max.size(maxSize={0})",
						MAX_EXCEL_ROW_BATCH_UPDATE);
			}

			// 轉物件
			List<SkuUpdateInfo> skuUpdateList = dataList.stream().map(strArr -> {
				SkuUpdateInfo bean = new SkuUpdateInfo();
				// 去空白
				bean.setOrderSouce(StringUtils.trimToEmpty(strArr[0]));
				bean.setOutId(StringUtils.trimToEmpty(strArr[1]));
				bean.setOldSkuId(StringUtils.trimToEmpty(strArr[2]));
				bean.setNewSkuId(StringUtils.trimToEmpty(strArr[3]));
				return bean;
			}).collect(Collectors.toList());

			// 2019.06.20 订单来源渠道 外部订单号 原货品条码 三個欄位重複得不更新
			Map<String, Long> groupbyKey = skuUpdateList.stream().collect(
					Collectors.groupingBy(bean -> bean.getDistinctKey(), Collectors.counting()));
			// 字段完全重覆
			Map<SkuUpdateInfo, Long> geoupByAll = skuUpdateList.stream()
					.collect(Collectors.groupingBy(bean -> bean, Collectors.counting()));

			// 回傳值
			AtomicInteger successCnt = new AtomicInteger(); // 成功筆數
			List<String> failList = com.google.common.collect.Lists
					.newArrayListWithExpectedSize(dataList.size());
			// 紀錄foreach的INDEX
			AtomicInteger index = new AtomicInteger();
			index.getAndIncrement(); // 從第二筆開始算
			// 成功的資料
			List<Integer> succesLine = com.google.common.collect.Lists
					.newArrayListWithExpectedSize(dataList.size());
			// for
			skuUpdateList.stream().forEach(skuUpdateInfo -> {
				index.getAndIncrement(); // 紀錄foreach的INDEX
				if (StringUtils.isBlank(skuUpdateInfo.getOutId())
						|| StringUtils.isBlank(skuUpdateInfo.getOldSkuId())
						|| StringUtils.isBlank(skuUpdateInfo.getOrderSouce())
						|| StringUtils.isBlank(skuUpdateInfo.getNewSkuId())) {
					failList.add("第" + index.get() + "条单据修改失败，原因：数据不存在");
					// 数据不存在
					return;
				}
				// 2019.06.20 订单来源渠道 外部订单号 原货品条码 三個欄位重複得不更新
				if (groupbyKey.get(skuUpdateInfo.getDistinctKey()) > 1
						|| geoupByAll.get(skuUpdateInfo) > 1) {
					failList.add("第" + index.get() + "条单据修改失败，原因：存在重复数据");
					return;
				}
				try {

					SkuOrder skuOrder = orderReadLogic.findSkuOrderBySkuUpdateInfo(skuUpdateInfo);

					updateSkuOrderCodeAndSkuId(skuOrder.getId(), skuUpdateInfo.getNewSkuId());

					successCnt.getAndIncrement();
					succesLine.add(index.get());
				} catch (JsonResponseException ex) {
					failList.add("第" + index.get() + "条单据修改失败，原因："
							+ opMessageSources.get(ex.getMessage()));
			...
			return Response.ok(ImmutableMap.of("successCnt", successCnt.get(), "failList",
					failList, "succesLine", succesLine));
		} catch (IOException | OpenXML4JException | ParserConfigurationException
				| SAXException e) {
