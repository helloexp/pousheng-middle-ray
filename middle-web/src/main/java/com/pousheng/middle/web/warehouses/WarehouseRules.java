...
import com.pousheng.middle.web.warehouses.dto.SimpleZone;
...
public class WarehouseRules {
    ...
    @RequestMapping(value = "/find/zones", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public List<SimpleZone> getAllZoneNames() {
        List<SimpleZone> zones = Lists.newArrayList();
        List<ShopChannelGroup> channelGroups = shopChannelGroupCacher.listAllShopChannelGroupCache();
        if (channelGroups != null && channelGroups.size() > 0) {
            for (ShopChannelGroup channel : channelGroups) {
                if ("mpos".equals(channel.getChannel())) {
                    List<ShopChannel> shopChannels = channel.getShopChannels();
                    if (shopChannels != null && shopChannels.size() > 0) {
                        for (ShopChannel shopChannel: shopChannels) {
                            SimpleZone simpleZone = new SimpleZone();
                            simpleZone.setZoneName(shopChannel.getZoneName());
                            simpleZone.setId(shopChannel.getZoneId());
                            zones.add(simpleZone);
                        }
                    }
                }
            }
