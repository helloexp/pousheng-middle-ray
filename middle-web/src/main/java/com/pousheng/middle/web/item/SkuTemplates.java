...
import com.pousheng.middle.web.excel.acvitity.ActivityItemRequest;
import com.pousheng.middle.web.excel.acvitity.AcvitityItemResponseBean;
import com.pousheng.middle.web.excel.acvitity.ImportAcvitityItemService;
...
public class SkuTemplates {
    ...
    @Autowired
    private ImportAcvitityItemService importAcvitityItemService;
    ...
    @ApiOperation("货品导入, 用于赠品活动")
    @RequestMapping(value = "/api/sku-template/for/gift/import", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public Response<AcvitityItemResponseBean> importGiftItem(ActivityItemRequest request){
        log.info("开始导入赠品商品文件:{}", request.getFilePath());
        Response<AcvitityItemResponseBean> acvitityItemResponseBeanResponse = importAcvitityItemService.importFile(request);
        log.info("导入赠品商品文件结束:{}, 结果:{}", request.getFilePath(), acvitityItemResponseBeanResponse);
        return acvitityItemResponseBeanResponse;
    }
