...
public class WarehouseAddressRuleWriteServiceImpl implements WarehouseAddressRuleWriteService {
    ...
    @Override
    public Response<Long> create(WarehouseAddressRule warehouseAddressRule) {
        try {
            warehouseAddressRuleDao.create(warehouseAddressRule);
            return Response.ok(warehouseAddressRule.getId());
        } catch (Exception e) {
            log.error("create warehouseAddressRule failed, warehouseAddressRule:{}, cause:{}", warehouseAddressRule, Throwables.getStackTraceAsString(e));
            return Response.fail("warehouse.address.rule.create.fail");
        }
    }
