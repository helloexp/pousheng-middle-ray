...
public class SyncVIPLogic {
    ...
    private static String deleteVipMsg = "Vip仓库映射已删除,仓库Id=%s";

    private static String deleteVipRuleMsg = "Vip仓库发货规则已删除,仓库Id=%s";

    private static  String deleteVipWarehouseKey ="deleteVipWarehouseKey_";
        ...
        try {
            ...
            if (refund.getRefundType().equals(MiddleRefundType.AFTER_SALES_RETURN.value())) {
                ...
                response = vipOrderReturnService.confirmReturnResult(refund.getShopId(), shopOrder.getOutId(),
                    null, refundExtra.getShipmentCorpName(), refundExtra.getShipmentSerialNo(),
            ...
            if (response.isSuccess()) {
                ...
                if (Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_RETURN.value())) {
                    update.setRefundFlag(refund.getRefundFlag() & (TradeConstants.MAX_REFUND_FLAG
                        - MiddleRefundFlagEnum.REFUND_SYN_THIRD_PLANT.getValue()));
                }
            } else {
    ...
    public void syncWarehouseMapping(Long shopId) {
        ...
        Boolean needPrintLog = Boolean.FALSE;
        try {
            ...
            for (Long warehouseId : warehouseMap.keySet()) {
                if (!storeList.contains(warehouseId)) {
                    //增加日志
                    addLogAndNotifyWhenDeleteOxoWarehouse(deleteVipMsg, warehouseId);
            ...
            for (Long warehouseId : rWarehouseIds.getResult()) {
                if (!storeList.contains(warehouseId)) {
                    //增加日志
                    addLogAndNotifyWhenDeleteOxoWarehouse(deleteVipRuleMsg, warehouseId);
                    needPrintLog = Boolean.TRUE;
                    //TODO 临时注释掉
            ...
            if (needPrintLog) {
                log.info("current.vip.query.store={}", JsonMapper.nonEmptyMapper().toJson(queryList));
            }
            if (needPushStock) {
    ...
    private void addLogAndNotifyWhenDeleteOxoWarehouse(String msg, Long warehouseId) {
        try {

            if(checkeIsToWrite(warehouseId)){
                return;
            }
            //发送钉钉消息
            dingTalkNotifies.addMsg(msg, warehouseId);
            OperationLog operationLog = new OperationLog();
            operationLog.setType(5);
            operationLog.setOperatorName("system");
            operationLog.setOperateId("-1");
            Map<Object, Object> content = Maps.newHashMap();
            content.put("type", "删除");
            content.put("warehouseId", warehouseId);
            ...
            if (response.isSuccess() && Objects.nonNull(response.getResult())) {
                content.put("warehouseContent", JsonMapper.nonDefaultMapper().toJson(response.getResult()));
            }
            content.put("msg", msg);
            operationLog.setContent(JsonMapper.nonDefaultMapper().toJson(content));
            operationLogWriteService.create(operationLog);
            this.writeLogToRedis(warehouseId);
        } catch (Exception e) {
            log.info("addLogAndNotifyWhenDeleteOxoWarehouse.error,warehouseId={},msg={},errormsg={}", warehouseId, msg,
                Throwables.getStackTraceAsString(e));
        }
    }

    private Boolean checkeIsToWrite(Long warehouseId){

        String  existFlag= jedisTemplate.execute(jedis -> {
            return jedis.get(deleteVipWarehouseKey+warehouseId);
        });
        if(Objects.nonNull(existFlag)){
            return Boolean.TRUE;
        }
    ...
    private void writeLogToRedis(Long warehouseId){

        jedisTemplate.execute(jedis -> {
            return jedis.setex(deleteVipWarehouseKey+warehouseId,60*60*12,warehouseId.toString());
        });
