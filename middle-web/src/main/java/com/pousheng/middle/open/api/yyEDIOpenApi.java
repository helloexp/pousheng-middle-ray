    ...
    public void receiveYYEDIShipmentCancelResult(@NotEmpty(message = "shipment.code.is.null") String shipmentCode,
        ...
        try {
            ...
            if (shipmentCode.startsWith(REFUND_SIGN)) {
                ...
                ShopOrder order = orderReadLogic.findShopOrderByCode(orderRefund.getOrderCode());

                //如果是云聚bbc的售后单取消，需通知云聚，抛出biz事件
                if (Objects.equals(order.getOutFrom(), MiddleChannel.YUNJUBBC.getValue())) {
                    YJSyncCancelRequest yyEdiResponse = new YJSyncCancelRequest();
                    //云聚 1 成功 2失败
                    yyEdiResponse.error_code(Objects.equals(syncSuccessOrderOperation, MiddleOrderEvent.SYNC_CANCEL_SUCCESS.toOrderOperation()) ? 1 : 2);
                    yyEdiResponse.error_info(description);
                    yyEdiResponse.order_sn(refund.getOutId().replace(MiddleChannel.YUNJUBBC.getValue() + "_", ""));
