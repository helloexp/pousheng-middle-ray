...
public class SkuTemplateSearchReadServiceImpl implements SkuTemplateSearchReadService {
    ...
    @Override
    public Response<List<SkuTemplate>> findByIds(List<Long> ids) {
        try {
            List<SkuTemplate> data = skuTemplateExtDao.findByIds(ids);
            return Response.ok(data);
        } catch (Exception e) {
            log.error("failed to find sku by  ids:({}), cause:({})", ids, e);
            return Response.fail("sku.template.find.fail");
        }
    }
