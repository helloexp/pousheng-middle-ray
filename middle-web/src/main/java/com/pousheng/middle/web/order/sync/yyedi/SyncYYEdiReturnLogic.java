...
public class SyncYYEdiReturnLogic {
    ...
    public YYEdiReturnInfo makeSyncYYEdiRefund(Refund refund) {
        ...
        if (!StringUtils.isEmpty(refund.getBuyerNote())) {
            refundInfo.setRemark(refund.getBuyerNote());
        }
        if (!StringUtils.isEmpty(refund.getSellerNote())) {
            refundInfo.setSellremark(refund.getSellerNote());
        }
        
        // XXX RAY POUS934 2019.04.16: 電商退貨單接口增加billsource參數
        refundInfo.setBillsource(ParameterWMS.BillSource.中台.getCode());
        //中台接单时间->中台接单时间：退货单-中台售后订单页面中的创建时间
        if(null != orderRefund.getCreatedAt()) {
            refundInfo.setBilldate(formatter.print(orderRefund.getCreatedAt().getTime()));
        }
        return refundInfo;
    }
	...
	public Response<Boolean> syncRefundCancelToYyEdi(Refund refund) {
		try {
			...
			Response<Boolean> updateStatusRes = refundWriteLogic.updateStatusLocking(refund, orderOperation);
			if (!updateStatusRes.isSuccess()) {
				log.error("refund(id:{}) operation :{} fail,error:{}", refund.getId(), orderOperation.getText(),
			...
			// XXX RAY POUS934 2019.04.16:  電商取消退貨單接口，增加companycode參數，標註，定單來源
			RefundExtra refundExtra = refundReadLogic.findRefundExtra(refund);
			if (refundExtra.getWarehouseId() != null) {
				Response<WarehouseDTO> response = warehouseClient.findById(refundExtra.getWarehouseId());
				if (!response.isSuccess()) {
					log.error("find warehouse by id :{} failed,  cause:{}", refundExtra.getWarehouseId(),
							response.getError());
					throw new ServiceException(response.getError());
				}
				WarehouseDTO warehouse = response.getResult();
				// 公司码
				yyEdiCancelInfo.setCompanycode(warehouse.getCompanyCode()); // 比照電商退貨單接口，傳出companycode
			}
			yyEdiCancelInfo.setBillsource(ParameterWMS.BillSource.中台.getCode());//参照电商取消退货单接口，传出billsource
			String response = sycYYEdiRefundCancelApi.doCancelOrder(yyEdiCancelInfo);
			YYEdiResponse yyEdiResponse = JsonMapper.nonEmptyMapper().fromJson(response, YYEdiResponse.class);
			if (Objects.equals(yyEdiResponse.getCode(), TradeConstants.YYEDI_RESPONSE_CODE_SUCCESS)) {
				// 同步调用成功后，更新售后单的状态
				Response<Boolean> updateSyncStatusRes = this.upateCancelRefundSuccess(refund, orderOperation,
						updateStatusRes);
			...
			// 同步调用成功后，更新售后单的状态
			this.updateCancelRefundFailed(refund);
			log.error("sync hk refund failed,refundId is({}) cause by({})", refund.getId(),
