 ...
 */
@Repository
public class SkuTemplateExtDao extends SkuTemplateDao {
    ...
    public List<SkuTemplate> findBySpuIds(List<Long> spuIds, Date updatedAt) {
        if (CollectionUtils.isEmpty(spuIds)) {
            return Collections.emptyList();
        }
        Map<String, Object> map = new HashMap<>();
        map.put("spuIds", spuIds);
        if (updatedAt != null) {
            map.put("updatedAt", updatedAt);
        }
        return sqlSession.selectList(sqlId("findBySpuIds"), map);
    }
