...
public class Refunds {
    ...
    @Autowired
    private RefundPostProcessor refundPostProcessor;
     ...
     * @return
     */
    @RequestMapping(value = "/api/refund/import", method = RequestMethod.GET,
    ...
    @RequestMapping(value = "/api/refund/import/test", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    //逆向单分页
    @RequestMapping(value = "/api/refund/paging", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    public Paging<RefundPaging> findBy(MiddleRefundCriteria criteria) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-PAGING-END param: criteria [{}] ,resp: [{}]", criteriaStr,
                JsonMapper.nonEmptyMapper().toJson(pagingRes));
        }
        return pagingRes.getResult();
    }

    //逆向单详情
    @RequestMapping(value = "/api/refund/{id}/detail", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    public MiddleRefundDetail detail(@PathVariable(value = "id") @PermissionCheckParam Long refundId) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-DETAIL-END param: refundId [{}] ,resp: [{}]", refundId,
                JsonMapper.nonEmptyMapper().toJson(detail));
        }
        return detail;
    }
	...
	public EditMiddleRefund edit(@RequestParam(required = false) @PermissionCheckParam Long refundId) {
		...
		if (Arguments.isNull(refundId)) {
			...
			// XXX RAY POUS925 2019.04.15: 新增快遞公司下拉選單
	...
	/**
	 * XXX RAY POUS925 2019.04.03: 新增快遞公司傳入值 
	 *    创建逆向单
	 *
     ...
     */
    @RequestMapping(value = "/api/refund/{id}/handle", method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @OperationLogType("完善或提交售后单")
    public void completeHandle(@PathVariable(value = "id") @OperationLogParam Long refundId,
                               @RequestBody EditSubmitRefundInfo editSubmitRefundInfo) {
        String editSubmitRefundInfoStr = JsonMapper.nonEmptyMapper().toJson(editSubmitRefundInfo);
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-HANDLE-START param: refundId [{}] editSubmitRefundInfo [{}]", refundId,
                editSubmitRefundInfoStr);
        }
        ...
        try {
            if (Objects.equals(refund.getRefundType(), MiddleRefundType.LOST_ORDER_RE_SHIPMENT.value())) {
                ...
                if (!Objects.equals(refund.getChannel(), MiddleChannel.VIPOXO.getValue()) && !refundReadLogic
                    .checkShipInfoUnique(editSubmitRefundInfo.getShipmentSerialNo())) {
                    throw new JsonResponseException("submit.ship.info.exists");
                }
                // 退货退款，换货，的发货仓和退货仓账套是否匹配325的校验
                if (Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_RETURN.value()) || Objects
                    .equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value())) {
                    ...
                    if (StringUtils.hasText(oriShipmentCode)) {
                        ...
                        checkShopOrWarehouse(oriShipment, refund, editSubmitRefundInfo.getWarehouseId(),
                            checkMatchCopanyCodeMap);
                        if (!checkMatchCopanyCodeMap.isEmpty()) {
                            if (checkMatchCopanyCodeMap.containsKey("refundCompanyIsNot325")) {
                                // 该退货仓没有退到指定的仓库，更新仓库id
                                String rulewarehouseid = checkMatchCopanyCodeMap.get("ruleRefundWareHouseId");
                                editSubmitRefundInfo.setWarehouseId(Long.parseLong(rulewarehouseid));
                            }
                            if (checkMatchCopanyCodeMap.containsKey("companyCodeNotMatch")) {
                                // 退货仓与订单店铺账套不一致，提示修改。
                                log.debug("shop companycode and warehouse companycode is not match {}, refundId is {}", refund.getId());
                ...
                if (Objects.equals(editSubmitRefundInfo.getOperationType(), 2)) {
                    //售后单对应发货单数量验证
                    if (Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value()) || Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_RETURN.value()) || Objects.equals(refund.getRefundType(), MiddleRefundType.REJECT_GOODS.value())) {
                        Map<String, String> codes = new HashMap<>();
                        if (validateShipment(refund, codes)) {
                            throw new InvalidException("refund.related.shipment.code={0},shipment.sku.code={1},quantity.error", codes.get("shipmentCode"), codes.get("shipmentSkuCode"));
                        }
                    }
                    ...
                    if (Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value())) {
                        result = refundWriteLogic.createOccupyShipments(editSubmitRefundInfo.getEditSubmitChangeItems(),
                            refund.getId());
                    }
                    if (result) {
                        //完善之后同步售后单到订单派发中心
        ...
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-HANDLE-END param: refundId [{}] editSubmitRefundInfo [{}]", refundId,
                editSubmitRefundInfoStr);
        }
    }
     ...
     */
    @RequestMapping(value = "/api/refund/batch/handle", method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public void completeHandle(@RequestParam(value = "refundIds") String data) {
        ...
        if (!Objects.equals(refundIds.size(), refunds.size())) {
            log.error("find refund by refund ids:{} result size not equal request id size:{}", refundIds,
                refunds.size(), refundIds.size());
        ...
        refunds = refunds.stream().filter(refund -> {
            ...
            if (!refundExtraMap.containsKey(TradeConstants.MIDDLE_REFUND_COMPLETE_FLAG)) {
            ...
            else if (Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_RETURN.value()) || Objects
                .equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value())) {
                ...
                if (!checkCompanyCodesMap.isEmpty()) {
                    ...
                    // 扩展需求：只要是在配置列表中的账套，退货仓都只能为对应规则中的仓库。2019-8月
                    if (checkCompanyCodesMap.containsKey("refundCompanyIsNot325")) {
                        log.debug("refund(id:{}) check refundWareHouse is not warehouse of rule", refund.getId());
                        return updateRefundWareHouseInfo(refund,checkCompanyCodesMap.get("ruleRefundWareHouseId"),checkCompanyCodesMap.get("ruleRefundWareHouseName"));
                    }
            ...
            if (Objects.equals(refund.getRefundType(), MiddleRefundType.LOST_ORDER_RE_SHIPMENT.value())) {
                ...
                if (!response.isSuccess()) {
                    ...
                    if (Objects.equals(refund.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value())) {
                        ...
                        if (result) {
                            Flow flow = flowPicker.pickAfterSales();
                            Integer targetStatus = flow.target(refund.getStatus(),
        ...
        if (!CollectionUtils.isEmpty(checkFailedRefundCodes)
            ...
            throw new InvalidException(
                "refund.batch.deal.fail.info(check.incomplete={0},handle.failed={1},sync.failed={2},not.match.companyCode={3})",
    ...
    @RequestMapping(value = "/api/refund/{id}/delete", method = RequestMethod.DELETE,
        produces = MediaType.APPLICATION_JSON_VALUE)
     ...
     */
    @RequestMapping(value = "/api/refund/{id}/wait/handle/sku", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    public List<WaitShipItemInfo> refundWaitHandleSku(@PathVariable("id") @PermissionCheckParam Long refundId) {
        ...
        if (Objects.equals(refund.getRefundType(), MiddleRefundType.LOST_ORDER_RE_SHIPMENT.value())) {
            ...
            for (RefundItem refundItem : refundLostItems) {
                if ((refundItem.getApplyQuantity() - (refundItem.getAlreadyHandleNumber() == null ? 0
                    : refundItem.getAlreadyHandleNumber())) > 0) {
                    this.waitShipItems(waitShipItemInfos, refundItem);
                }
            }
            if (log.isDebugEnabled()) {
                log.debug("API-REFUND-WAIT-HANDLE-SKU-END param: refundId [{}] ,resp: [{}]", refundId,
                    waitShipItemInfos);
            }
    ...
    public void syncHkCancelRefund(@PathVariable(value = "id") @PermissionCheckParam Long refundId) {
        ...
        if (!Objects.equals(refund.getRefundType(), MiddleRefundType.LOST_ORDER_RE_SHIPMENT.value())) {
            ...
            if (Objects.equals(refund.getStatus(), MiddleRefundStatus.SYNC_HK_FAIL.getValue())) {
                ...
                Response<Boolean> updateSyncStatusRes = refundWriteLogic.updateStatusLocking(refund,
                    syncSuccessOrderOperation);
                if (!updateSyncStatusRes.isSuccess()) {
                    log.error("refund(id:{}) operation :{} fail,error:{}", refund.getId(),
                        syncSuccessOrderOperation.getText(), updateSyncStatusRes.getError());
    ...
    public void confirmReceived(@PathVariable(value = "id") @PermissionCheckParam Long refundId) {
        ...
        Response<Boolean> cancelRes1 = refundWriteLogic.updateStatusLocking(refund,
    ...
    public void createCustomerServiceNote(@PathVariable("id") @OperationLogParam Long id,
                                          @RequestParam("customerSerivceNote") String customerSerivceNote) {
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-CREATECUSTOMERSERVICENOTE-START param: id [{}] ,customerSerivceNote [{}]", id,
                customerSerivceNote);
        }
        refundWriteLogic.addCustomerServiceNote(id, customerSerivceNote);
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-CREATECUSTOMERSERVICENOTE-END param: id [{}] ,customerSerivceNote [{}]", id,
                customerSerivceNote);
        }
    }
     ...
     * @param id 售后单id
     */
    @RequestMapping(value = "/api/refund/{id}/cancel/on/change", method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
     ...
     */
    @RequestMapping(value = "/api/refund/{id}/manual/confirm/refund", method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public void confirmRefund(@PathVariable("id") @OperationLogParam Long refundId) {
        ...
        if (!flow.operationAllowed(sourceStatus, MiddleOrderEvent.REFUND.toOrderOperation())) {
            log.error("refund(id:{}) current status:{} not allow operation:{}", refund.getId(), refund.getStatus(),
                MiddleOrderEvent.REFUND.toOrderOperation().getText());
        ...
        if (!updateStatusRes.isSuccess()) {
            log.error("refund(id:{}) operation :{} fail,error:{}", refund.getId(),
                MiddleOrderEvent.REFUND.toOrderOperation().getText(), updateStatusRes.getError());
     ...
     * @param refundId 售后单id
     */
    @RequestMapping(value = "/api/refund/{id}/manual/confirm/return", method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public void confirmReturn(@PathVariable("id") @OperationLogParam Long refundId) {
        ...
        if (!flow.operationAllowed(sourceStatus, MiddleOrderEvent.RETURN.toOrderOperation())) {
            log.error("refund(id:{}) current status:{} not allow operation:{}", refund.getId(), refund.getStatus(),
                MiddleOrderEvent.RETURN.toOrderOperation().getText());
            throw new JsonResponseException("order.status.invalid");
        }
        Response<Boolean> updateStatusRes = refundWriteLogic.updateStatusLocking(refund,
        ...
        if (!updateStatusRes.isSuccess()) {
            log.error("refund(id:{}) operation :{} fail,error:{}", refund.getId(),
                MiddleOrderEvent.RETURN.toOrderOperation().getText(), updateStatusRes.getError());
            throw new JsonResponseException("update.refund.error");
        }
        if (log.isDebugEnabled()) {
     ...
     */
    @RequestMapping(value = "/api/refund/{id}/is/out/from/jd", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    public boolean isOutFromJD(@PathVariable("id") Long refundId) {
        ...
        ShopOrder shopOrder = orderReadLogic.findShopOrderById(orderRefund.getOrderId());
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-ISOUTFROMJD-END param: refundId [{}] ,resp: [{}]", refundId,
                Objects.equals(shopOrder.getOutFrom(), MiddleChannel.JD.getValue()));
        }
     ...
     * @return
     */
    @RequestMapping(value = "/api/refund/{code}/already/refund/fee", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    public int getAlreadyRefundFee(@PathVariable("code") String orderCode,
                                   @RequestParam("shipmentId") String shipmentCode,
                                   @RequestParam(required = false) Long refundId,
                                   @RequestParam(value = "list") String list) {
        if (log.isDebugEnabled()) {
            log.debug(
                "API-REFUND-GETALREADYREFUNDFEE-START param: orderCode [{}] shipmentCode [{}] refundId [{}] list [{}]",
                orderCode, shipmentCode, refundId, list);
        }
        List<RefundFeeData> refundFeeDatas = JsonMapper.nonEmptyMapper().fromJson(list,
            JsonMapper.nonEmptyMapper().createCollectionType(List.class, RefundFeeData.class));
        int num = refundReadLogic.getAlreadyRefundFee(orderCode, refundId, shipmentCode, refundFeeDatas);
        if (log.isDebugEnabled()) {
            log.debug(
                "API-REFUND-GETALREADYREFUNDFEEx-END param: orderCode [{}] shipmentCode [{}] refundId [{}] list [{}] ,resp: [{}]",
                orderCode, shipmentCode, refundId, list, num);
        }
        return num;
    }

    @RequestMapping(value = "/api/refund/max/refund/fee", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public void editReceiverInfos(@PathVariable("id") @OperationLogParam Long id,
                                  @RequestParam(required = false) String buyerName,
                                  @RequestBody MiddleChangeReceiveInfo middleChangeReceiveInfo) {
        String middleChangeReceiveInfoStr = JsonMapper.nonEmptyMapper().toJson(middleChangeReceiveInfo);
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-EDITRECEIVERINFOS-START param: id [{}] buyerName [{}] middleChangeReceiveInfo [{}]",
                id, buyerName, middleChangeReceiveInfoStr);
        }
        middleRefundWriteService.updateReceiveInfos(id, middleChangeReceiveInfo);
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-EDITRECEIVERINFOS-END param: id [{}] buyerName [{}] middleChangeReceiveInfo [{}]", id,
                buyerName, middleChangeReceiveInfoStr);
        }
    }
    ...
    public Response<Boolean> confirmDoneForLost(@PathVariable("id") @OperationLogParam Long id) {
        ...
        Refund refund = refundReadLogic.findRefundById(id);
        Response<Boolean> r = refundWriteLogic.updateStatusLocking(refund,
    ...
    public Response<Boolean> confirmAfterSaleOccupyShipments(
        @PathVariable("refundId") @OperationLogParam Long refundId) {
        Refund refund = refundReadLogic.findRefundById(refundId);
        ...
        for (OrderShipment orderShipment : orderShipments) {
            ...
            shipmentWiteLogic.updateOccupyShipmentTypeByShipmentId(orderShipment.getShipmentId(),
                ShipmentOccupyType.CHANGE_N.name());
    ...
    public Response<Boolean> cancelAfterSaleOccupyShipments(
        @PathVariable("refundId") @OperationLogParam Long refundId) {
        Refund refund = refundReadLogic.findRefundById(refundId);
        //非skx占库单取消使用原来占库单取消的逻辑
        //skx占库单取消需要到skx取消一把
    ...
    public Response<List<MiddleAfterSaleInfo>> findAfterSaleIds(@PathVariable("code") String code) {
        ...
        List<Refund> afterSales = refunds.stream().filter(Objects::nonNull)
            ...
            .filter(
                it -> !Objects.equals(it.getStatus(), MiddleRefundStatus.RETURN_DONE_WAIT_CREATE_SHIPMENT.getValue()))
            .filter(it -> !Objects
                .equals(it.getStatus(), MiddleRefundStatus.RETURN_DONE_WAIT_CONFIRM_OCCUPY_SHIPMENT.getValue()))
            ...
            .filter(it -> (Objects.equals(it.getRefundType(), MiddleRefundType.AFTER_SALES_CHANGE.value()) || Objects
                .equals(it.getRefundType(), MiddleRefundType.LOST_ORDER_RE_SHIPMENT.value())))
            .collect(Collectors.toList());
        //添加售后单信息
        ...
        middleAfterSaleInfos.add(middleAfterSaleInfo);
        if (log.isDebugEnabled()) {
            log.debug("API-REFUND-FINDAFTERSALEIDS-END param: code [{}] ,resp: [{}]", code,
                JsonMapper.nonEmptyMapper().toJson(middleAfterSaleInfos));
        }
        return Response.ok(middleAfterSaleInfos);
    }

    /**
     * 售后单类型修改 售后仅退款-->拒收单 修改售后单类型需要去判断当前售后单类型（MiddleRefundType.AFTER_SALES_REFUND）是否为售后仅退款
    ...
    public Response<Boolean> updateRefundType(@PathVariable("id") Long id) {
        ...
        if (!Objects.equals(refund.getStatus(), MiddleRefundStatus.WAIT_HANDLE.getValue()) ||
            ...
            throw new JsonResponseException("reject.only.can.be.total.created");
        }
        ...
        for (ShipmentItem shipmentItem : shipmentItems) {
            ...
            for (RefundItem refundItem : refundItemList) {
                if (Objects.equals(refundItem.getSkuOrderId(), shipmentItem.getSkuOrderId()) && Objects.equals(refundItem.getApplyQuantity(), shipmentItem.getShipQuantity())) {
                    valid = true;
                    break;
                }
            }
            if (!valid) {
                log.error("current refund id {} can not change type to reject_goods", id);
                throw new JsonResponseException("refund.status.not.allow.current.operation");
            }
        }
            ...
            for (ShipmentItem shipmentItem : shipmentItems) {
                if (Objects.equals(x.getSkuOrderId(), shipmentItem.getSkuOrderId())) {
                    shipmentItem.setRefundQuantity(
                        (shipmentItem.getRefundQuantity() == null ? 0 : shipmentItem.getRefundQuantity()) + x
                            .getApplyQuantity());
                }
            }
        });
     ...
     * @return true修改成功
     */
    @RequestMapping(value = "/api/refund/edit/change/items/quantity", method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
    public Response<Boolean> editRefundExchangeItems(
        @RequestBody EditRefundShipmentItemsInfo editRefundShipmentItemsInfo) {
        ...
        Map<String, Integer> editSkuCodeAndApplyQuantityMap = editRefundChangeItems.stream().
            ...
            Collectors.toMap(SkuCodeUtil::getCombineCode, RefundItem::getApplyQuantity));
        //将申请数量修改
        ...
        for (RefundItem refundItem : existRefundChangeItems) {
            ...
            if (editSkuCodeAndApplyQuantityMap.containsKey(SkuCodeUtil.getCombineCode(refundItem))) {
                ...
                refundItem.setCleanFee(
                    editSkuCodeAndApplyQuantityMap.get(SkuCodeUtil.getCombineCode(refundItem)) * refundItem
                        .getCleanPrice());
            }
            newRefundChangeItems.add(refundItem);
        }
        Map<String, String> refundExtra = refund.getExtra();
        refundExtra.put(TradeConstants.REFUND_CHANGE_ITEM_INFO,
            JsonMapper.nonEmptyMapper().toJson(newRefundChangeItems));
     ...
     */
    @RequestMapping(value = "/api/refund/validate/fee/for/change", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
     ...
     * @param id 售后单id
     */
    @RequestMapping(value = "/api/refund/{id}/exchange/to/refund", method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public Response<Boolean> exchangeToRefund(@PathVariable("id") @OperationLogParam Long id) {
        ...
        if (!openShopExtraMap.containsKey(TradeConstants.EXCHANGE_PULL) || !Objects.equals(
            openShopExtraMap.get(TradeConstants.EXCHANGE_PULL), "Y")) {
        ...
        //查询天猫单个换货单详情
        OpenClientAfterSaleExchangeService afterSaleExchangeService = afterSaleExchangeServiceRegistryCenter
        ...
        Response<OpenClientAfterSale> openClientAfterSaleResponse = afterSaleExchangeService.findByAfterSaleId(
            originRefund.getShopId(), outerId);
        if (!openClientAfterSaleResponse.isSuccess()) {
            log.error("sync after sale by open shop id:{} outer after after id:{} fail,error:{}",
                originRefund.getShopId(), originRefund.getOutId(), openClientAfterSaleResponse.getError());
        ...
        if (Objects.equals(originRefund.getStatus(), MiddleRefundStatus.CHANGE_SYNC_HK_SUCCESS.getValue()) && !Objects
            .equals(openClientAfterSale.getStatus(), OpenClientAfterSaleStatus.EXCHANGE_CLOSED)) {
            throw new JsonResponseException("tmall.refund.not.close");
        }
        if (!Objects.equals(openClientAfterSale.getStatus(), OpenClientAfterSaleStatus.EXCHANGE_TO_REFUND) && !Objects
            .equals(openClientAfterSale.getStatus(),
                OpenClientAfterSaleStatus.EXCHANGE_CLOSED)) {//天猫换货状态不是 换货关闭，转退货退款（请退款）
        ...
        if (Objects.equals(originRefund.getStatus(), MiddleRefundStatus.WAIT_SHIP.getValue())) {
            ...
            List<OrderShipment> cancelIngList = orderShipments.stream().filter(Objects::nonNull)
                .filter(orderShipment -> Objects
                    .equals(orderShipment.getStatus(), MiddleShipmentsStatus.SYNC_HK_CANCEL_ING.getValue()))
            ...
            if (!updateExtraRes.isSuccess()) {
                log.error("update refund extraMap(refundId:{}) fail,error:{}", originRefund.getId(),
                    updateExtraRes.getError());
            }
            List<OrderShipment> orderShipmentList = orderShipments.stream().filter(Objects::nonNull)
                .filter(orderShipment -> !Objects
                    .equals(orderShipment.getStatus(), MiddleShipmentsStatus.CANCELED.getValue()))
                .collect(Collectors.toList());
     ...
     * @param filePath
     */
    @ApiOperation("批量导入店铺指定退货仓规则")
    @RequestMapping(value = "/api/refund/targetWarehouse/rule/import", method = RequestMethod.POST,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public void targetWarehouseImportFile(String filePath) {
        if (!StringUtils.hasText(filePath)) {
            log.error("faild import targetWarehouse rule file(path:{}) ,cause: file path is empty", filePath);
            throw new JsonResponseException("file.path.is.empty");
        }
        PoushengCompensateBiz biz = new PoushengCompensateBiz();
        biz.setBizType(PoushengCompensateBizType.IMPORT_SHOP_REFUND_TARGET_WAREHOUSE_RULE.toString());
        biz.setContext(filePath);
        biz.setBizId(UUID.randomUUID().toString());
        biz.setStatus(PoushengCompensateBizStatus.WAIT_HANDLE.toString());
     ...
     * @return
     */
    @ApiOperation("查询指定退货仓规则导入文件的处理记录")
    @RequestMapping(value = "/api/refund/targetWarehouse/rule/import/result/paging", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public Paging<PoushengCompensateBiz> createTargetWarehouse(@RequestParam(required = false, value = "pageNo") Integer pageNo,
                                                @RequestParam(required = false, value = "pageSize") Integer pageSize) {
        PoushengCompensateBizCriteria criteria = new PoushengCompensateBizCriteria();
        criteria.setPageNo(pageNo);
        criteria.setPageSize(pageSize);
        criteria.setBizType(PoushengCompensateBizType.IMPORT_SHOP_REFUND_TARGET_WAREHOUSE_RULE.toString());
        Response<Paging<PoushengCompensateBiz>> response = poushengCompensateBizReadService.pagingForShow(criteria);
        if (!response.isSuccess()) {
            throw new JsonResponseException(response.getError());
        }
    ...
    @ApiOperation("批量设置店铺售后指定退货仓规则下载")
    @GetMapping("/api/refund/targetWarehouse/rule/template/download")
    public void loadTargetWarehouseTemplate(HttpServletRequest request, HttpServletResponse response) {
        try {
            String fileName = "批量设置店铺售后指定退货仓规则模板.xlsx";
            List<String> headers = Lists.newArrayList("销售店铺(必填：账套-外码)", "发货仓账套(必填,例如: 301)", "退货仓(必填：账套-外码)");
            ExportLoadTemplateUtil.exportTemplateExcel(fileName,"批量设置店铺售后指定退货仓", headers,request, response);
        } catch (Exception e) {
            log.error("downlaod targetWarehouse rule template failed. cause: {}", Throwables.getStackTraceAsString(e));
            throw new JsonResponseException(500, "下载批量设置店铺售后指定退货仓规则模板失败！");
        }
    }
     ...
     * @return
     */
    @ApiOperation("分页查询指定退货仓规则")
    @GetMapping(value = "/api/refund/targetWarehouse/rule/paging")
    public Paging<RefundWarehouseRules> warehouseRulesPaging(@RequestParam(required = false, value = "pageNo") Integer pageNo,
                                                      @RequestParam(required = false, value = "pageSize") Integer pageSize,
                                                      @RequestParam(required = false, value = "saleShopName")  String saleShopName) {
        if (log.isDebugEnabled()) {
            log.debug("api-targetWarehouse-rule-paging param: pageNo [{}],pageSize [{}],saleShopName [{}]", pageNo,pageSize,saleShopName);
        }
        ...
        RefundWarehouseRules criteria = new RefundWarehouseRules();
        if (StringUtils.hasText(saleShopName)) {
            criteria.setOrderShopName(saleShopName);
        }
        Response<Paging<RefundWarehouseRules>> response = refundWarehouseRulesReadService.pagingRules(pageNo,pageSize,criteria);
        if (!response.isSuccess()) {
            throw new JsonResponseException(response.getError());
        }
        return response.getResult();
     ...
     * 删除指定退货仓规则
     */
    @ApiOperation("删除指定退货仓规则")
    @RequestMapping(value = "/api/refund/targetWarehouse/rule/{id}/delete", method = RequestMethod.DELETE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @OperationLogType("删除指定退货仓规则")
    public void deleteWarehouseRule(@PathVariable(value = "id") @OperationLogParam Long ruleId) {
        if (log.isDebugEnabled()) {
            log.debug("API-REFUNDWAREHOUSERULE-DELETE-START param: Id [{}]", ruleId);
        }
            refundWriteLogic.deleteRefundWarehouseRules(ruleId);
        if (log.isDebugEnabled()) {
            log.debug("API-REFUNDWAREHOUSERULE-DELETE-END param: Id [{}]", ruleId);
        }
    }
     ...
     * 根据ruleid获取售后退货仓规则
     */
    @ApiOperation("根据规则id获取指定退货仓规则")
    @RequestMapping(value = "/api/refund/targetWarehouse/rule/{id}/get", method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public MiddleRefundWarehouseRuleDto getByRuleId(@PathVariable(value = "id") @OperationLogParam Long ruleId) {
        MiddleRefundWarehouseRuleDto ruleDto = refundReadLogic.findRefundWarehouseRuleDtoById(ruleId);
        return ruleDto;
    }

    /**
     * 提交编辑售后退货仓规则
     * @param refundWarehouseRules
     */
    @ApiOperation("更新售后退货仓规则")
    @RequestMapping(value = "/api/refund/edit/targetWarehouse/rule/commit", method = RequestMethod.PUT,produces = MediaType.APPLICATION_JSON_VALUE)
    public void updateRefundWarehouseRule(@RequestBody RefundWarehouseRules refundWarehouseRules) {
        refundWriteLogic.updateRefundWarehouseRule(refundWarehouseRules);
    }

    /**
     * 根据配置的退货仓规则获取指定的退货仓
     * @param orderShopId
     * @param shipmentCompanyCode
     * @return
     */
    @ApiOperation("根据配置的退货仓规则获取指定的退货仓")
    @GetMapping(value = "/api/refund/get/targetWarehouse/by/refundRule",produces = MediaType.APPLICATION_JSON_VALUE)
    public WarehouseDTO queryWarehouseByRefundRule(@RequestParam(value = "orderShopId") Long orderShopId,
                                                   @RequestParam(value = "shipmentCompanyCode")  String shipmentCompanyCode){
        Response<RefundWarehouseRules> refundWarehouseRule = refundWarehouseRulesReadService.findByShopIdAndShipmentCompanyId(orderShopId, shipmentCompanyCode);
        if(!refundWarehouseRule.isSuccess()){
            throw new JsonResponseException(refundWarehouseRule.getError());
        }
        WarehouseDTO warehousedto = null;
        if(!Objects.isNull(refundWarehouseRule.getResult())){
            warehousedto = refundReadLogic.findWarehouseByCodeandCompanyId(refundWarehouseRule.getResult().getRefundWarehouseCode());
        }
        return warehousedto;
    }
    ...
    private EditMiddleRefund makeEditMiddleRefund(Long refundId) {
        ...
        // XXX RAY POUS925 2019.06.19 新增快遞公司下拉選單
        setExpressNameList(editMiddleRefund);
    ...
    private void waitShipItems(List<WaitShipItemInfo> waitShipItemInfos, RefundItem refundItem) {
        ...
        waitShipItemInfo.setWaitHandleNumber(
            refundItem.getApplyQuantity() - (refundItem.getAlreadyHandleNumber() == null ? 0
                : refundItem.getAlreadyHandleNumber()));
    ...
    // 修改退货仓信息
    private Boolean updateRefundWareHouseInfo(Refund refund, String ruleWarehouseId, String ruleWarehouseName) {
        Map<String, String> refundExtraMap = refund.getExtra();
        if (refundExtraMap.containsKey(TradeConstants.REFUND_EXTRA_INFO)) {
            ...
            // 修改退货仓id 
            refundExtra.setWarehouseId(Long.parseLong(ruleWarehouseId));
            refundExtra.setWarehouseName(ruleWarehouseName);
    ...
    private void checkShopOrWarehouse(Shipment shipment, Refund refund, Long warehouseid,
                                      Map<String, String> compCodeMap) {
        ...
        if (Objects.equals(shipment.getShipWay().toString(), TradeConstants.MPOS_SHOP_DELIVER)) {
            ...
            if (checkWarehousedto != null) {
                getShipcompanyId = checkWarehousedto.getCompanyId();
            }
        }
        if (StringUtils.hasText(getShipcompanyId)) {
            Long saleShopId = refund.getShopId();
            Response<RefundWarehouseRules> refundWarehouseRule = refundWarehouseRulesReadService.findByShopIdAndShipmentCompanyId(saleShopId, getShipcompanyId);
            if (refundWarehouseRule.isSuccess()) {
                if (refundWarehouseRule.getResult() != null) {
                    // 匹配到指定退货仓规则，将不符合规则中的仓的数据标记
                    addIsNot325Company(refund, warehouseid, refundWarehouseRule.getResult().getRefundWarehouseCode(), compCodeMap);
                } else {
                    // 场景2：如果来源单的发货店账套不是325，则校验来源单的店铺账套与退货仓的账套是否一致
                    String companyCode = refundWriteLogic.getCompanyCodeOrderShop(saleShopId);
                    companyCodeNotMatch(companyCode, refund, warehouseid, compCodeMap);
                }
            } else {
                //查询异常,返回
                log.debug("get refundWarehouseRule failed,refundId:{}", refund.getId());
                compCodeMap.put("companyCodeNotMatch", refund.getRefundCode());
            }
        } else {
            //查询异常,返回
            log.debug("get getShipcompanyId is null,refundId:{}", refund.getId());
            compCodeMap.put("companyCodeNotMatch", refund.getRefundCode());
        }
    }
    ...
    private void addIsNot325Company(Refund refund, Long warehouseid,String ruleWarehouseCode, Map<String, String> compCodeMap) {
        WarehouseDTO warehousedto = refundReadLogic.findWarehouseByCodeandCompanyId(ruleWarehouseCode);
        Long ruleWarehouseId = warehousedto.getId();
        if (!Objects.equals(warehouseid, ruleWarehouseId)) {
            log.error("warehouse of refund(id:{}) is not refunds_warehouse_rules的指定退货仓", refund.getId());
            compCodeMap.put("refundCompanyIsNot325", refund.getRefundCode());
            compCodeMap.put("ruleRefundWareHouseId", ruleWarehouseId.toString());
            compCodeMap.put("ruleRefundWareHouseName", warehousedto.getWarehouseName());
        }
    }

    private void companyCodeNotMatch(String orderCompanyCode, Refund refund, Long warehouseId,
                                     Map<String, String> compCodeMap) {
        ...
        if (checkWarehousedto != null) {
            ...
            if (!Objects.equals(checkWarehousedto.getCompanyId(), orderCompanyCode)) {
                log.error("companyCode of refund(id:{}) warehouse and saleShop is not match ", refund.getId());
                compCodeMap.put("companyCodeNotMatch", refund.getRefundCode());
            }
        }
    }
	...
	/**
	 * XXX RAY POUS925 2019.04.15: 設定快遞公司下拉選單
	 * 
	 * @param bean DTO
	 */
	private void setExpressNameList(EditMiddleRefund bean) {
		...
		if (resp.isSuccess()) { // 只取快遞公司中文
			bean.setRefundExpressNameList(
					resp.getResult().stream().map(ExpressCode::getName).collect(Collectors.toList()));
		}
	}
    @RequestMapping(value = "/api/refund/oxo/complete", method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    public void completeOxo(@RequestParam Long refundId) {
        refundPostProcessor.postProcessorAfterCreated(refundId);

    }

    //校验售后单与对应发货单的数量
    private Boolean validateShipment(Refund refund, Map<String, String> codes) {
        RefundExtra refundExtra = refundReadLogic.findRefundExtra(refund);
        List<RefundItem> refundItemList = refundReadLogic.findRefundItems(refund);
        //发货单是否有效
        Shipment shipment = shipmentReadLogic.findShipmentByShipmentCode(refundExtra.getShipmentId());
        ...
        for (RefundItem refundItem : refundItemList) {
            for (ShipmentItem shipmentItem : shipmentItems) {
                if (Objects.equals(refund.getRefundType(), MiddleRefundType.REJECT_GOODS.value())) {
                    if (Objects.equals(refundItem.getSkuOrderId(), shipmentItem.getSkuOrderId()) && refundItem.getApplyQuantity() != (shipmentItem.getShipQuantity() == null ? 0 : shipmentItem.getShipQuantity())) {
                        log.error("current refund id {} apply quantity is greater than shipment id {} ship quantity", refund.getId(), shipment.getId());
                        codes.put("shipmentCode", shipment.getShipmentCode().toString());
                        codes.put("shipmentSkuCode", shipmentItem.getSkuCode().toString());
                        return true;
                    }
                } else {
                    if (Objects.equals(refundItem.getSkuOrderId(), shipmentItem.getSkuOrderId()) && refundItem.getApplyQuantity() > (shipmentItem.getShipQuantity() == null ? 0 : shipmentItem.getShipQuantity())) {
