...
import com.pousheng.middle.web.excel.warehouse.WarehouseTagImportService;
...
public class ChannelInventory {
    ...
    @Autowired
    private WarehouseTagImportService warehouseTagImportService;
    ...
    public Response<String> importTags(ImportRequest request) {
        log.info("开始导入仓库标签,文件:{}", request.getFilePath());
        Response<String> response = warehouseTagImportService.importFile(request);
        log.info("导入仓库标签结束,文件:{}", request.getFilePath());
        return response;
    }
